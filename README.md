...
Supervision watches the Herdr lifecycle stream directly. It holds **one** long-lived connection to `HERDR_SOCKET_PATH` for the whole session carrying a single acknowledged `events.subscribe`; Herdr 0.8.2 answers exactly one request per connection, so the subscription set is fixed and global, every supervisor is multiplexed over it, and each `session.snapshot` is a separate short-lived read taken only when something needs one. The child is pinned by `terminal_id` plus the whole `agent_session` and anchored on the pane `revision` its bind snapshot observed, because Herdr reuses pane IDs. A pane move is followed only on atomic evidence plus a fresh matching occupant; a move that is not this child's is a reconciliation trigger rather than a conclusion, because pane IDs are reused. Replay deduplication is the pane `revision` alone — monotonic per pane occupancy, which a proven move preserves — and never a position in the retained log, whose head can be dropped. Every accepted event is validated on its own fields at the protocol boundary; a malformed one drops the connection instead of disappearing. On reconnect a supervisor resumes silently only when the fresh snapshot proves nothing advanced; otherwise it emits one high-priority `evidence_gap` and adopts the status and revision that snapshot proves, because the outage's individual transitions may have scrolled out of the log. There is no polling fallback: a socket that cannot be restored leaves the supervisor visibly degraded.
