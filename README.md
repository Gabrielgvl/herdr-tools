...
Every qualified profile-backed launch receives one launcher-generated, run-owned Markdown handoff path under the endpoint-private Herdr Tools state namespace. The record has fixed headings (`Status`, `Summary`, `Changes`, `Verification`, `Blockers`, `Continuation`), a `done`/`blocked`/`cancelled`/`failed` status, and `None` when no files changed. Callers never select the path.

This is a Tools-owned orchestration gate, not a Herdr core lifecycle veto. For managed Pi, Claude, and Devin runs, a matching terminal observation remains unaccepted and `herdr_wait` keeps polling until the current artifact validates. `completed` gates both `idle` and `done`; `terminal` gates every terminal outcome. AGY remains refused. Turn-level cancel or interrupt does not end a run; a runtime-authored cancelled fallback requires confirmed run-level cancellation or authoritative exit. Restart restoration is deferred, so shutdown leaves unresolved records `recovery_pending`. See [the delegation spec](docs/specs/profile-backed-delegation.md) and [ADR-031](docs/decisions/031-universal-disk-handoff.md).
