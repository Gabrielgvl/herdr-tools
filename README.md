...
`deploy/systemd/herdr-tools-daemon.service` is the `systemd --user` unit for the durable supervisor daemon ([spec](docs/specs/durable-supervisor.md)). The unit is committed inert: this work never runs `systemctl --user enable` or `start`. Activation is a separate owner gate — write `~/.config/herdr-tools/daemon.env` with `HERDR_SOCKET_PATH` (and `PATH` if `node` is not on the unit's default), then `systemctl --user enable --now herdr-tools-daemon.service`. Until activation, daemon clients refuse `DAEMON_UNAVAILABLE` — a visible switch, not a drift into in-process behavior.
