...
## Spec launches, categories, and candidates

`herdr_launch` is spec-only: the caller authors one or more `specs`, each carrying a kebab-case `label` (≤ 12 chars, unique per request), `instructions`, a typed `assignment` of exactly `objective`/`scope`/`verification`, an optional `category`, and an optional replica `count`. The request also carries a required `supervisionDigest` with `doneWhen`, `constraints`, and optional `readOnly`. The reviewer receives `doneWhen` and `constraints`; `readOnly` drives a code-owned workspace check. Profiles, roles, `overrides`, and raw `kind`/`argv`/`env` fields no longer exist — the strict schema rejects them.

```text
herdr_launch({"name":"reviewer","specs":[{"label":"inspect","instructions":"Review the working tree for correctness regressions","assignment":{"objective":"Inspect the current changes","scope":"Read-only review of the working tree","verification":"Report findings with file and line references"}}],"supervisionDigest":{"doneWhen":["Findings reported with file:line references"],"constraints":["none"]}})
```

Without `transportBypass`, each spec is evaluated by Jev (`jev-latest`) in one `systemOne` call — instruction quality, assignment verifiability, category choice, and per-candidate resource judgments — then admitted by deterministic policy: the catalog's ordered `{runner, model}` candidate chain for the resolved category (`frontier`, `balanced`, `cheap` in `herdr-profiles/catalog.yaml`) is filtered by availability, the first eligible candidate is selected, and the rest remain the fallback chain inside `agent_start`. A candidate compiles through `compileCandidateContract` into runner-specific argv — Pi gets `--tools`/`--extension`/`--skill` selections from the reviewed pools, Claude gets allowed/disallowed tools and plugin dirs, and AGY and Devin consume no pool fields. Admission failures are typed abstentions (`low_confidence`, `no_assignments`, `catalog_unavailable`, `invalid_response`, `authentication_unavailable`, `transport_failed`, `aborted`) with zero launch effects; every per-spec decision is appended once to `.herdr/router/decisions.jsonl` under the session project root before the first child mutation.

Put a positive resource requirement in its own sentence. The current explicit-requirement parser suppresses an entire punctuation-delimited clause when the clause contains a negative term, so `must use read, but do not use write` can lose the `read` requirement.

`transportBypass` is a caller-supplied replay token for a recorded transport abstention. Replay checks fixed markers, binding equality, and an unkeyed SHA-256 digest over the supplied receipt. It does not look up the decision log or recompile the receipt's configuration. Treat the digest as an integrity checksum, not authentication.

Children are named exactly `{name}-{spec.label}-{N}` (`task-worker-1`, `task-inspect-2`), and `count > 1` gives each replica an isolated worktree under `.herdr/worktrees/`. The result is `launch` for a single expanded child or `launch_batch` with `outcome` of `abstained`/`launched`/`partial`/`failed` plus one entry per child. `partial` retains mixed outcomes as they stand: nothing is closed, rolled back, or resubmitted — inspect surviving children by name and delegate the remainder explicitly. The public `herdr_jobs` projection exposes `candidateName` (the candidate that actually started the child) and, when fallback changed the selection, `requestedCandidateName`.

Spec evaluation resolves its TypeSafe credential through `resolveTypesafeApiKey`: an explicit key, then the `TYPESAFE_API_KEY` environment variable, then the `typesafe` `api_key` entry in the Pi auth store (`~/.pi/agent/auth.json`) — the same bridge the supervision reviewer uses.
...
The normative contract is [the `herdr_launch` section of SPEC.md](SPEC.md#herdr_launch); the design rationale is [ADR-035](docs/decisions/035-spec-launches-jev-compiled-harnesses.md).
...
herdr_launch({"name":"reviewer","specs":[{"label":"inspect","instructions":"Review the working tree for correctness regressions","assignment":{"objective":"Inspect the current changes","scope":"Read-only review of the working tree","verification":"Report findings with file and line references"}}],"supervisionDigest":{"doneWhen":["Findings reported"],"constraints":["none"]}})
...
- `herdr-profiles/catalog.yaml` is the reviewed catalog: category chains of `{runner, model}` candidates, runner defaults, resource pools, quota sources, and availability policy inputs that spec evaluation and contract compilation consume.
