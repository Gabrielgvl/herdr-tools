...
Each spec is evaluated by Jev (`jev-latest`) in one `systemOne` call — instruction quality, assignment verifiability, category choice, and per-candidate resource judgments — then admitted by deterministic policy: the catalog's ordered `{runner, model}` candidate chain for the resolved category (`frontier`, `balanced`, `cheap` in `herdr-profiles/catalog.yaml`) is filtered by availability, the first eligible candidate is selected, and the rest remain the fallback chain inside `agent_start`. A candidate compiles through `compileCandidateContract` into runner-specific argv — Pi gets `--tools`/`--extension`/`--skill` selections from the reviewed pools, Claude gets allowed/disallowed tools and plugin dirs, and AGY and Devin consume no pool fields. Admission failures are typed abstentions (`low_confidence`, `no_assignments`, `catalog_unavailable`, `invalid_response`, `authentication_unavailable`, `transport_failed`, `aborted`) with zero launch effects; every per-spec decision is appended once to `.herdr/router/decisions.jsonl` under the session project root before the first child mutation.
