...
- `src/mcp/` holds the MCP host only: startup gating and the `cwd`/`signal` capability proxy (`host.ts`), schema publication and bounded result/error mapping (`adapter.ts`), and stdio wiring and lifecycle (`run.ts`). `src/mcp-server.ts` is the argument-free entry emitted to `dist/src/mcp-server.js`.
