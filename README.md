...
`worker-pi`, the Claude profiles, and the Devin profiles are the local qualification targets; Claude passed a fresh live requalification with exact recipient-generated body, identity, and single-request socket evidence (see the unblock report). AGY profiles remain catalog metadata and their fallback edges are refused: this hotfix refuses AGY launch, prompt, and steer with `AGY_UNQUALIFIED`; no AGY child is created. `worker-devin` is the implementation default and `reviewer-devin` the review default (see below); both run the Devin runtime under the same strict exact-identity contract as Pi and Claude. The original full integration runner and its AGY cases remain separate, preserved, and **NOTQUALIFIED** here.
...
AGY's reduced-assurance provisional/strengthening contract is retained in ADR-021/022 but is not reachable in the local hotfix. AGY requests fail before resource preparation (`AGY_UNQUALIFIED`); Claude requests follow the same exact-identity, fresh-reread, one-send contract as Pi, and no blocked-recipient live receipt is inferred from a refusal. The preserved original full gate remains **NOTQUALIFIED**.
...
Wakes are best effort and never retried. Pi extension hosts wake by steered custom context. MCP hosts route each wake by the manager pane's own agent kind: `devin` and `pi` hosts self-prompt through `agent.prompt` with the full provenance envelope (`kind: supervision`, or `kind: wait` for a settled `herdr_wait`), `claude` hosts send only the Channels notification, and every other or unproven kind stays inert. `manager-claude` declares no development channels, so it passes no `--dangerously-load-development-channels` opt-in and raises neither the organization-policy nor the missing-MCP-server warning at startup. Nothing is lost without an inbound wake: every material event carries an opaque ID, `herdr_jobs get` returns the pending ones and marks exactly those observed, and `herdr_jobs list` and the active-job footer show unobserved counts.
