...
A supervisor is reserved before the first topology mutation for every launch, including a launch with no `initialPrompt` and a launch into an existing pane. After readiness proves the complete child identity, `herdr_launch` binds the supervisor immediately, before optional focus or assignment. Binding drains queued evidence while the public job remains `reserved`, then commits `request.targetIds: [paneId]` and bound state together. A queued settlement or failed bind throws `SUPERVISION_UNCONFIRMED`, sends no focus or prompt, registers no recipient, performs no retry or cleanup, and leaves the child and binding evidence available for inspection.

Assignment confirmation is separate from supervision. If the single acknowledged initial prompt is not semantically confirmed, launch throws `LAUNCH_FAILED` with `causeCode: "PROMPT_UNCONFIRMED"`, `assignmentState: "unconfirmed"`, and the exact pane ID and retained supervisor job ID. Inspect that pane with `herdr_inspect` and the supervisor with `herdr_jobs get`. Do not relaunch, resend the assignment, auto-send Enter, close or reuse the pane, register a recipient, or continue dependent success assertions. The active supervisor remains responsible for the child's lifecycle. A successful launch returns its stable supervisor job ID.
...
The child is pinned by `terminal_id` plus the whole `agent_session` and anchored on the pane `revision`. Pane IDs are reused, so thin events trigger reconciliation rather than conclusions. A full event revision jump or same-revision status contradiction emits a visible high-priority `evidence_gap`; a higher valid snapshot revision emits a source-`snapshot` gap even when its endpoint status is unchanged, then adopts that status and revision. Malformed, duplicate, contradictory, or lower-revision target-local snapshot evidence cannot settle a live supervisor. Proven pane moves rebase to the destination's pane-local revision. A degraded live supervisor remains visible and keeps semantic-review ownership.
