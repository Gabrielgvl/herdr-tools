...
### Durable supervisor daemon (inactive)

`deploy/systemd/herdr-tools-daemon.service` is the `systemd --user` unit for the durable supervisor daemon ([spec](docs/specs/durable-supervisor.md), [ADR-038](docs/decisions/038-durable-supervisor.md)). The unit is committed inert: this work never runs `systemctl --user enable` or `start`. Until activation, daemon clients refuse `DAEMON_UNAVAILABLE` — a visible switch, not a drift into in-process behavior.

The caller surface is exactly three daemon-backed tools — `herdr_launch`, `herdr_run`, `herdr_status` — the universal harness surface: Claude and Devin reach them natively over MCP while their direct registrations remain, and every harness — including Pi, whose only path this is — reaches them through the executor→MCP gateway once registered. `herdr_launch` takes `{ task, idempotencyKey }` (the key is required; an identical retry is a replay, a different Task under a reused key is `IDEMPOTENCY_KEY_CONFLICT`). `herdr_run` is a discriminated union on `action`: `observe`, `reconcile`, `transfer`, `claim`, `ack`. `herdr_status` is the read-only projection — daemon health, your runs and intents, your per-session mailbox's unread event IDs and path, one bounded event body per `eventId`; it never acks and never mutates. Mailbox flow: `herdr_status` read → act → `herdr_run` `ack` (an atomic unread→acked rename, idempotent on retry).

Native follow-ups to children use the installed CLI signature exactly: `herdr agent prompt <TARGET> <TEXT>` (positional; no stdin or file flag). Large content never travels in argv — the manager writes an owner-only file `herdr-handoffs/<runId>/followups/<seq>.md` (0600) and `<TEXT>` is one short pointer line, e.g. `herdr follow-up <runId>/followups/<seq>.md`; the child reads the file with its own `read` tool. For a `devin`-kind child the manager re-reads the pane immediately before the send and sends only on fresh `idle`/`done`; `working`, `blocked`, unknown, or unproven defers, never sends.

#### Owner activation runbook (N5.3 — owner-only, ordered)

Every step below is an explicit owner action at cutover; this repository performs none of them.

1. **Merge** the durable-API branch with the unit inactive — merged clients keep refusing `DAEMON_UNAVAILABLE`; the cutover stays a visible switch.
2. **Install the build.** The installed checkout `~/.pi/agent/extensions/herdr-tools` fast-forwards and rebuilds through the autoupdate timer, or run `npm ci && npm run build:mcp` there manually. `dist/src/mcp-server.js` (the executor payload's `args`) and `dist/src/daemon/main.js` (the unit's `ExecStart`) must exist.
3. **Owner writes `~/.config/herdr-tools/daemon.env`** with `HERDR_SOCKET_PATH=/home/gabriel/.config/herdr/herdr.sock` — matching the socket path the executor payload carries — plus `PATH` if `node` is not on the unit's default.
4. **Install and start the unit:** `install -D -m644 -t ~/.config/systemd/user ~/.pi/agent/extensions/herdr-tools/deploy/systemd/herdr-tools-daemon.service && systemctl --user daemon-reload && systemctl --user enable --now herdr-tools-daemon.service`.
5. **Register the executor gateway — three calls** from `deploy/executor/herdr-mcp-integration.json` inside `executor_execute`, after the negative probe proves nothing is live (`deploy/executor/README`): `tools.executor.mcp.addServer(...)` → `tools.executor.coreTools.connections.create(...)` → `tools.executor.coreTools.connections.refresh(...)`.
6. **Run the committed positive probe** (`deploy/executor/README`): the `getServer` readback matches the payload, `tools.search` returns exactly `herdr.org.default.herdr_{launch,run,status}`, and `herdr_status` with `caller: { paneId, projectRoot }` answers — or returns a bounded `DAEMON_UNAVAILABLE` if the daemon is down.
7. **Apply the global Pi tool pins and the executor trio lanes, then restart installed MCP.** Global Pi and every profile lane pin exactly the five native tools (`read`, `bash`, `edit`, `write`, `ask_user_question`) — the prepared `~/.pi/agent` change and the catalog/profile changes, prepared-not-installed in N3.4. Apply `deploy/executor/remove-direct-herdr-claude.diff` and `remove-direct-herdr-devin.diff` so the Claude and Devin lanes also resolve the three tools through the gateway — those diffs ride this cutover, never earlier, because before the gateway is live they would break still-live managers' only Herdr reach. Restart installed MCP/plugin sessions to load the amended surface.
8. **Land the prepared `herdr-manager` recipe and repin the bundle** per `deploy/executor/herdr-manager-recipe.md`: apply its edits at the canonical external source, then `npm run generate:skill-bundles` and commit the regenerated copy together with the `herdr-skill-bundles.json` pin.
9. **Devin permission posture (recorded C9 finding, not optional).** Any supervised `devin` owner or manager must run with `--permission-mode dangerous` — the default `auto` mode stalls forever on the interactive MCP-approval dialog during a mailbox-hint turn — or a pre-approval mechanism must ship first. This is also the N5.3 ordering point for the §11 hint-body/Devin-surface reconciliation recorded in ADR-038: after the removal diffs land, Devin resolves `herdr_status` through the gateway and the hint body is actionable as written.
10. **Enable production hints and soak.** If the N5.2 code flip has not already landed, the production daemon entry gains `hintKinds: ["pi","claude","devin"]` in the same change that removes the Channels branch and capability — enablement is a code flip, not a config knob, and the shipped default stays empty until then (the mailbox is the durable path either way). Then soak on a disposable session: `test/integration/fixtures/daemon-disposable.mjs` with `HERDR_TOOLS_DISPOSABLE_HINT_KINDS` is the only non-production place a non-empty qualified set may be injected. Confirm the hint → mailbox → `ack` flow end to end there before relying on hints in production.
...
An exact supervisor is pinned by `terminal_id` plus the whole `agent_session` and anchored on the pane `revision`. AGY reaches this state only after strengthening. Pane IDs are reused, so thin events trigger reconciliation rather than conclusions — `pane_exited` included: its exit fact names no occupant, so it can never prove `process_exit` (that violation kind persists only for historical records). A full event revision jump or same-revision status contradiction emits a visible high-priority `evidence_gap`; a higher valid snapshot revision emits a source-`snapshot` gap even when its endpoint status is unchanged, then adopts that status and revision. Malformed, duplicate, contradictory, or lower-revision target-local snapshot evidence cannot settle a live supervisor. Proven pane moves rebase to the destination's pane-local revision. A degraded live supervisor remains visible and keeps semantic-review ownership.

Working starts are silent. The manager is woken for a completed work cycle, a block, reviewer attention, evidence or sink degradation and recovery, identity replacement or loss, release, pane close, reconciliation failure or recovery, and an evidence gap. A child that has been working continuously for the configured cadence is reviewed by Jev (`typesafe/jev-latest`, the `SUPERVISION_REVIEWER_MODEL` constant). One `systemOne` call per review judges six independent predicates plus a reason choice over bounded V2.1 evidence: the Task's `objective`, `doneWhen`, and `constraints` (never `scope`), a compact runner trace, Git workspace state, supplemental terminal lines, metadata, and previous-review state. The public schema has no `progressMarkers`, so that reviewer field is empty. Porcelain renames parse from either status column. The local safety scan is pattern-based; `safe` means no configured detector matched, not that arbitrary credentials are proven absent. A review whose work cycle ends mid-flight is abandoned rather than stored. Reviewer transport, authentication, HTTP, and malformed-response failures are silent and retry at the next cadence; evidence-read and sink failures degrade visibly.
...
This is a Tools-owned orchestration gate, not a Herdr core lifecycle veto. For managed runs, a matching terminal observation remains unaccepted and `herdr_wait` keeps polling until the current artifact validates. `completed` gates both `idle` and `done`; `terminal` gates every terminal outcome. Turn-level cancel or interrupt does not end a run; a runtime-authored cancelled fallback requires confirmed run-level cancellation or authoritative exit. Shutdown leaves unresolved records `recovery_pending`.

The same manager session can inspect one retained run through the daemon's `herdr_run observe` operation (the executor → MCP surface): it returns the `resumeHandoff` observation plus the intent that recorded the run and its unread event IDs, and refuses `DAEMON_UNAVAILABLE` while the unit is inactive. The command accepts no session or storage argument: caller identity is the native session of the injected Herdr pane — the pane the inherited `HERDR_*` variables name — verified as that pane's live occupant against the authoritative snapshot, and the store is the endpoint-private namespace. It only reports the persisted Task, lifecycle, exact current child identity, and trusted artifact evidence. It never sends input, relaunches a worker, transfers ownership, accepts a delivery, or restores supervision. Executor caller binding is unsupported. Identity is environment-derived here exactly as it is for every herdr-tools call: a same-user process sharing the Herdr environment and socket resolves as whichever pane its injected variables name, so the check is native-session-only, not per-process. The refusals that are delivered: no session or identity argument is accepted, and legacy records without provenance, wrong or duplicate manager sessions, changed children, and unavailable or changed artifacts fail closed.

See [ADR-031](docs/decisions/031-universal-disk-handoff.md).
