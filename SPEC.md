...
**Read-only handoff recovery.** After a build, `node scripts/resume-handoff.mjs RUN_UUID` may be invoked from native Herdr bash. It accepts no session, owner, or storage arguments: the caller pane is the injected Herdr pane, and its native manager session is resolved from that pane's verified live occupant in the authoritative snapshot, with the store in the endpoint-private namespace. As for every herdr-tools call, same-user processes sharing the Herdr environment and socket are trusted — the binding is the injected pane's verified live session, not the calling process. Recovery succeeds only for the same unique native manager session and exact retained child and artifact evidence. It returns an observation with `observationOnly: true`, `supervisionRestored: false`, `replayed: false`, and `ownershipTransferred: false`. It never invokes launch, prompt delivery, ownership mutation, or supervisor registration. Legacy or malformed provenance, another or duplicate manager session, a changed child, and an unavailable or digest-mismatched artifact are refusals.
