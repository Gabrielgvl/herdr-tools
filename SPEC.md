...
**One session connection.** Supervision holds one long-lived connection to `HERDR_SOCKET_PATH` for the whole manager session, carrying a single acknowledged `events.subscribe` and multiplexed across every supervisor. Herdr 0.8.2 answers exactly one request per connection, so the subscription set is fixed — `pane.updated`, `pane.closed`, `pane.exited`, `pane.moved`, `pane.agent_detected` — which also rules out per-pane `pane.agent_status_changed`, and every `session.snapshot` is a unary read on its own short-lived connection taken only when something needs one. `pane.updated` carries a full `PaneInfo`, so it is the authoritative status channel.

**Identity and anchoring.** A supervisor pins `pane_id`, `terminal_id`, the agent name and kind, and the whole four-part `agent_session`, and anchors on the pane `revision` its bind snapshot observed. Pane IDs are reused across the durable event log, so a thin event carrying only a pane ID is a reconciliation trigger — a fresh `session.snapshot` decides — never a conclusion. Every accepted event is validated on its own fields at the protocol boundary: a malformed known event drops the connection rather than being routed to no observer, and a non-atomic `pane_moved` never reaches a supervisor. Events are delivered to supervisors through one ordered chain, with every observer call isolated per observer so one failing supervisor cannot cost its siblings an event. Replay deduplication is the pane `revision` alone: it is monotonic per pane occupancy and a proven move preserves the occupancy, so it survives a move rewriting the routing key, while any position in the retained log would not — that log drops entries from its head. A pane move is followed only when an atomic move event and a fresh authoritative occupant both prove terminal and agent-session continuity. A move that does not leave this supervisor's pane, or whose own record does not prove this occupant, is a reconciliation trigger rather than a conclusion, because pane IDs are reused; only a provably-ours move with an unconfirmable destination settles `identity_lost` and wakes.

**Reconnect.** On reconnect the monitor re-bootstraps and each supervisor refolds from its anchor. Resuming silently is allowed only when the reconnect snapshot proves the child's lifecycle did not advance; otherwise exactly one high-priority `evidence_gap` is emitted, the status and revision that snapshot proves are adopted, and supervision continues. The adoption is not optional: the outage's individual transitions may have scrolled out of the retained log, so the snapshot is the only authority on where the child actually is. If the socket cannot be restored the supervisor stays visibly degraded and retries with bounded jittered backoff. There is no polling fallback.
