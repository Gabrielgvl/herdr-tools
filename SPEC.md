...
```text
{
  ...
  supervisionDigest: {                          // REQUIRED at request level; only doneWhen/constraints enter reviewer assignment evidence
    ...
    readOnly?: boolean                          // optional code-owned Tier-0 workspace claim; absent means false
  }
}
```
...
- **Bypass receipts.** The public request schema accepts a caller-supplied `transportBypass` token. `createBypassReceipt` mints one only when its caller asserts that a `transport_failed` abstention was recorded and binds it to `{caller, specRevision, policyRevision, launchIdentity}`. `replayBypassReceipt` checks that binding, fixed transport-abstention markers, and an unkeyed SHA-256 digest over the supplied receipt, then returns the receipt's admitted result without re-running Jev or `compileCandidateContract`. Replay does not look up the persisted abstention or compare the supplied configuration with catalog/compiler output. The digest detects accidental corruption but does not authenticate caller-controlled receipt data.
