...
- `prompt` delivers to `idle`, `done`, and `working` targets through one direct `agent prompt --stdin` submission. A working target receives the text exactly like an explicit steer — the target runtime decides how submitted input lands (Pi steers the running turn, Devin queues it for the composer flush) — while the envelope `kind` and `route` still record the declared prompt semantics. `blocked` targets are refused with `TARGET_BLOCKED`: a pane awaiting input could consume the text as its answer, and callers that deliberately want that use explicit `steer`.
...
**Purpose:** Launch a named Pi, Claude, or AGY agent from a strict profile and return authoritative launch state. **No profile, no launch.**
...
```text
{
  ...
  assignment: {                              // required for every Pi, Claude and AGY launch
...
- `herdr_launch` accepts only a validated named Pi, Claude, or AGY profile and typed overrides; raw kind, argv, and env launch fields are rejected.
...
- normal prompt delivery to a working target and refusal while blocked;
...
- Communication distinguishes normal prompt, explicit steer, named keys, cancel, and interrupt; a normal prompt to a working target submits through the same direct path an explicit steer uses; steer never synthesizes an interrupt; turn control binds one key to one stable working identity and independently verifies the outcome.
