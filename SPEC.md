...
Mutation results include authoritative post-state when the operation has one. For prompt/steer and launch initial-prompt deliveries, the typed submission acknowledgement is the atomic mutation evidence and the follow-up read is optional observation. A post-state and success-row state are retained only after the full captured pane/terminal/name/kind/agent_session identity matches; a replacement is omitted from authoritative fields and may appear only as bounded observation evidence. For a closed resource, the post-state is the authoritative containing context and a list of removed IDs, because the closed resource can no longer be read. Tool renderers show a compact row by default and may show structured details when expanded.
...
- Profile body sources are created before topology mutation. Real Herdr protocol 20 `agent_started` envelopes can intermittently omit identity fields, so launch treats only the fields actually supplied by start as authoritative and runs one bounded, read-only launch identity-preflight window (target approximately 5 seconds with short polling) before any initial-prompt dispatch or recipient registration. Every sample freshly reads snapshot, `agent get`, and pane, then joins that one coherent sample only with the start-supplied fields; missing components are never carried across samples. A complete start identity may fill fields omitted by that sample, while a missing start session must be supplied by that same sample. Any supplied contradiction against start or within a sample fails immediately; timeout or caller abort fails closed with bounded evidence. The same captured complete identity is used for the optional assignment, acknowledgement, post-state observation, and session-bound recipient registration, even when no initial prompt is requested. The assignment is wrapped in the mandatory v1 envelope and submitted at most once through `agent prompt --stdin` without `--wait`; this readiness preflight is not a prompt retry. A parsed `cli:agent:prompt` / `agent_prompted` response with the exact captured identity, `interactive_ready:true`, and safe `revision` confirms atomic acceptance. Herdr's optional working-state observation is not part of delivery acknowledgement: `screen_detection_skipped:true`, idle/done/blocked state, or an unchanged/older revision is reported as observation rather than treated as a prompt failure. The launch never sends a recovery Enter, never invokes a runtime hook, never falls back during polling, never retries submission, and never duplicates prompt bytes. A missing, malformed, killed, or mismatched identity/acknowledgement fails closed and leaves created resources visible.
- After a confirmed assignment, paired authoritative agent/pane reads are best-effort identity-bound observation. Launch details retain `initialPromptSubmission` and `initialPromptObservation`; an unavailable, stale, or replaced identity observation omits the replacement `postState` and never registers or describes it as the launched recipient. A launch without an initial prompt returns only the identity-classified fresh post-start state. Recipient registrations persist pane ID, terminal ID, agent name, agent kind, and the complete session object; optional `agent_id` is diagnostic only and never authorizes attachment delivery.
