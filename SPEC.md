...
Every cross-pane text delivery made by `herdr_communicate` (`prompt` or `steer`) and every `herdr_launch.assignment` is wrapped in this mandatory recipient-visible envelope:
...
Every `herdr_launch` child — the one explicit child or each dispatched Batch child — reserves supervision before its first topology mutation in phase `supervision_reserve`. A reservation failure refuses launch with `SUPERVISION_UNAVAILABLE` and `effectCertainty: "absent"`. Pi and Claude retain strict complete native-session readiness and exact binding before optional focus or assignment, including launches into existing panes. Every launch carries the mandatory typed `assignment`, so there is no promptless launch path. Assignment confirmation remains a separate launch-success gate. Supervision is observation and notification only: it never mutates the child, never gates its work, and never becomes a public tool.

**Pi and Claude bind exactly.** Phase `supervision_bind` follows readiness and precedes focus and prompt dispatch. Binding validates a fresh authoritative snapshot, drains queued evidence while the public view remains `reserved`, and commits the selected profile, selected kind, and `request.targetIds: [exactPaneId]` before publishing bound `active` or `degraded` state. A queued closure, release, replacement, identity loss, or failed commit makes bind reject as `SUPERVISION_UNCONFIRMED`. It rolls back request fields to `targetIds: []`, releases only the unbound reservation, and performs no focus, prompt, recipient registration, retry, or child cleanup. The child and binding evidence remain available for inspection. Profile fallback stays inside `agent_start`, before Pi or Claude binding and before AGY provisional publication.

**AGY binds provisionally, then strengthens.** The fixed non-sensitive `--prompt-interactive` bootstrap is the last AGY start argument and establishes the native AGY session before the real assignment. A coherent pre-prompt readiness sample must prove one idle AGY occupant's pane, terminal, name, kind, sequence, and revision; the existing provisional path accepts a pre-existing native `agent_session` without putting the assignment in argv. The registry then commits and publishes the AGY-only `provisional` state on the same mutation chain. After exactly one prompt submission, one observer-backed strengthening task takes a fresh authoritative snapshot, requires the official full native session and lifecycle advancement on the same identity, and drains every event admitted during the provisional window in arrival order. Only then may it atomically publish `active` or `degraded`, `request.targetIds: [exactPaneId]`, and exact coverage. A move, replacement, contradiction, duplicate, malformed record, failed read, missing session, sequence failure, revision regression, or settlement rejects strengthening and retains the provisional supervisor.

After exact Pi or Claude bind succeeds, a later focus, prompt, recipient, or rendering failure never releases or cancels the supervisor. After a prompt might have taken effect, any transport, acknowledgement, semantic-confirmation, identity, or strengthening failure retains the child, running supervisor, evidence, and recovery handles. The launch never retries assignment or start, falls back, sends Enter, cleans up, releases the reservation, registers a recipient or attachment capability, or authorizes dependent work. The only fallback remains an exact non-killed, untruncated pre-interactive `agent_start_failed` result followed by fresh authoritative proof that no agent exists. A successful launch returns the stable supervisor job ID.
...
**Review cadence.** A child that has been working continuously for `wait.reviewCadenceMinutes` is reviewed with exactly `openai-codex/gpt-5.6-luna` at thinking `max`, on a UTF-8-bounded prompt carrying only what is new since the previous completed review. A review whose work cycle ends while its transcript read or model call is in flight is abandoned rather than stored or announced. Results store silently; `stalled`, `blocked`, `risk`, `appears_complete`, and `unknown` wake the manager and the supervisor stays active. A reviewer failure enters one visible degraded episode and retries at the next cadence, and the first success afterwards notifies recovery once. The reviewer never starts a Herdr agent and never selects a substitute model.
...
**Purpose:** Launch managed agents from strict validated Profiles and return authoritative launch state. A request naming a `profile` is an explicit single launch; a request carrying `assignment` with no `profile` is a Batch — the Jev-backed Router decides Roles, Profiles, and Fan-out, and each resulting Assignment expands into children that each walk this same launch lifecycle. **Every executed child uses a validated Profile.**

**Input:** a strict union of two object variants. Unknown fields and invalid discriminant combinations are rejected on both.

```text
// Explicit single launch — a named Profile is required
{
  ...
  assignment: {                              // required for every Pi, Claude, Devin and AGY launch
    objective: string,                       // non-empty, no NUL
    scope: string,                           // non-empty, no NUL
    verification: string                     // non-empty, no NUL
  },
  assignmentDelivery?: "inline" | "attachment", // default: inline
  supervisionDigest: {                       // required on every launch — authorial done-when/constraints the supervisor's reviewer judges against
    doneWhen: string[],                      // 1-8 non-empty strings of at most 240 characters, no NUL
    constraints: string[]                    // same bound; author ["none"] when unconstrained
  }
}

// Auto Batch — the Router chooses Roles, Profiles, and Fan-out
{
  name: string,                              // required caller base name for derived child names
  placement?:                                // same union; existing_pane is single-child only
    { mode: "same_tab" }                    // default
    | { mode: "new_tab", tabLabel: string }
    | { mode: "existing_pane", target: TargetRef },
  label?: string,                            // base pane label; children are suffixed -{role}-{N}
  cwd?: string,                              // default: current Pi cwd
  focus?: boolean,                           // default: false
  assignment: {                              // same strict shape as the explicit variant
    objective: string,                       // non-empty, no NUL
    scope: string,                           // non-empty, no NUL
    verification: string                     // non-empty, no NUL
  },
  assignmentDelivery?: "inline" | "attachment", // default: inline
  supervisionDigest: {                       // required on every Batch launch — same strict shape as the explicit variant
    doneWhen: string[],                      // 1-8 non-empty strings of at most 240 characters, no NUL
    constraints: string[]                    // same bound; author ["none"] when unconstrained
  }
}
...
The auto variant has no `profile` and no `overrides`: no primary Profile has been named to receive overrides, so `overrides` without a named Profile is invalid, as is any other field outside the auto shape. The discriminator is field presence, not field value — a present `profile` field is explicit mode even when malformed (null, empty, non-string, or a present `undefined`), so it fails explicit validation rather than silently routing; only an absent `profile` is a Batch. There is no `profile: "auto"` sentinel, caller-supplied Assignment list, or separate public routing tool, and a literal Profile named `auto`, if it exists, remains an ordinary explicit Profile name.

Raw `kind`, `argv`, and `env` launch fields are rejected on both variants. Profiles are resolved from the manager session cwd using bundled, user, then nearest-project discovery; arbitrary valid named profiles are allowed. Each profile pins exactly `pi`, `claude`, `agy`, or `devin` and supplies typed runtime fields, timeout, persistence, and an ordered fallback graph of at most four reachable profiles. `researcher-agy` is the advisory research default, uses `gemini-3.8-flash-low`, and declares exactly `researcher-agy -> researcher-claude -> researcher-devin -> researcher-pi`; `scout-agy` also uses `gemini-3.8-flash-low` and declares `scout-agy -> scout-claude -> scout-devin -> scout-pi`, while `worker-devin -> worker-pi -> worker-claude` is the implementation chain. The same effective catalog feeds the Router in a Batch, so a Router-chosen Profile never widens what can be launched.

Rules:

These rules are the single launch lifecycle. An explicit call runs it once; every dispatched Batch child runs the identical lifecycle independently under the Batch rules below.

- `name` is required and, in explicit mode, must be unique according to an authoritative agent listing (`INVALID_INPUT` on a collision). The extension never generates a name or silently renames a collision. In a Batch, `name` is the caller-chosen base from which exact child names are derived and validated per the Batch rules; explicit-mode collisions keep `INVALID_INPUT` while Batch children use their own typed codes.
- Default placement splits the current pane to the right in the current tab, with no focus change, current Pi cwd, and a pane label equal to `name` unless `label` is supplied. `new_tab` and `existing_pane` retain their existing explicit semantics. In a Batch the same defaults apply per child, with the derived `-${role}-{N}` label suffixes described below.
- Pi and Claude profile body sources are created before topology mutation and appended through their existing system-prompt channels. An AGY profile body remains catalog metadata and is never passed to AGY. AGY receives repository `AGENTS.md` through native discovery and requires one visible, self-contained, provenance-wrapped `assignment`. Immediately before each `agent start` invocation, launch records a monotonic start-budget basis. When that attempt is selected, one condition-based, read-only readiness loop uses the remainder of the same absolute 120,000 ms startup budget; start return, sampling, and polling never reset it. Every sample is a fresh ordered `api snapshot` → `agent get` → `pane get` sequence and is evaluated only after all three reads complete, using that sample plus identity fields actually supplied by the selected `agent_started` record. No record or field carries across samples. Fixed-whitelist model-visible evidence resets before each sample, then retains each completed current-sample read; if pane-get fails, the completed snapshot and agent-get projections remain available without evaluating readiness. Duplicate target records, malformed non-null metadata, contradictory identity aliases, or replacement evidence is terminal. Missing/null noncontradictory startup metadata is pending only inside readiness. Every non-null lifecycle field from start or sample records is shape-validated: known `agent_status`, safe non-negative `state_change_seq`/`revision`, and boolean diagnostic `screen_detection_skipped`. Valid start lifecycle values are earlier process-start observations and are never merged into or compared with a later sample. Every launch carries the mandatory typed `assignment`, so every launch requires the idle lifecycle baseline and lifecycle agreement below, and no promptless launch path exists. The assignment is validated for exact shape -- exactly `objective`, `scope`, and `verification`, each a non-empty string without NUL, with extras and any legacy `initialPrompt`/`initialPromptDelivery` field refused -- before mutation, reservation, attachment publication, or any CLI call. It is then rendered once as `objective`, `scope`, `verification` in that fixed order with fixed labels, so the same assignment always renders byte-identically, and the rendered payload's UTF-8 byte length is the single authority for the selected delivery bound. No AGY fallback is pruned from a launchable chain, because no child can be started without its assignment.
...
- Overrides apply only to the requested primary profile. AGY accepts only typed `model` and scope-normalized `addDirs` overrides. Every fallback uses its own untouched defaults, including runtime, model, source, timeout, and permissions. Profile `timeoutMinutes` is task policy; each Herdr start attempt uses one absolute valid 120000 ms start-plus-readiness budget, while the CLI subprocess receives a small execution margin.
- AGY argv is fixed to `--model <model> --mode <profile.mode> --dangerously-skip-permissions`, where the required fixed profile mode is exactly `plan` or `accept-edits`, followed by scope-normalized profile `--add-dir` values and the Herdr-Tools-owned recipient attachment directory, then the final `--prompt-interactive "Initialize this interactive session and reply with exactly AGY_READY."` bootstrap. This fixed non-sensitive bootstrap establishes the native session before the real assignment, which remains exclusively in the later provenance-wrapped `agent prompt --stdin` input. AGY accepts no prompt file, `--agent`, arbitrary argv, arbitrary env, or non-persistent session mode; `mode` is not an override.
...
- Fallback attempts reuse the resolved pane; the ordered reachable profile chain is deterministic and capped at four. An exhausted chain stops and reports bounded attempt evidence; no profile is improvised.
...
#### Router decision (auto Batch)

An auto call first runs the same fail-closed launch gate check, request validation, rendered-assignment size bound, and prompt-transport compatibility preflight as an explicit call — all before the Router decision — and releases the preflight gate lease before inference, so nothing is held across the Jev call. It then performs exactly one routing decision. An explicit named-Profile call performs zero Router work: no catalog projection, no Jev client construction, no `TYPESAFE_API_KEY` requirement, and no decision-log append.

- **Catalog and state.** The same manager/session-rooted effective catalog that explicit launch uses is loaded once and projected to exactly five allowlisted fields per Profile — `name`, `description`, `runner`, `model`, `timeout` — sorted by exact name. Jev receives only the caller's `assignment` and this flat projection: no Profile bodies, tools, permissions, argv, environment, source paths, or other caller-supplied state fields. A Role is derived from a Profile name as the segment before the first `-`, or the whole name when no `-` exists; there is no Role allowlist, no candidate filtering (AGY and manager/promoter Profiles stay visible), and no Fan-out cap. A catalog that cannot be authoritatively loaded or projected — an absent or failing loader, a malformed effective map, or unreadable scopes — is a `catalog_unavailable` Abstain decided before the Jev call; blocked or missing entries are never invented back.
- **Questions.** The single `systemOne` request carries three independent questions per derived Role with deterministic IDs `${role}_useful`, `${role}_count`, and `${role}_profile`, none referencing another question's answer, and no repair, retry, or model-listing calls. The literal question text is fixed routing contract:
...
${role}_count (score, five fixed criteria describing semantic agent counts one through five):
...
${role}_profile (choice among that Role's projected candidates):
...
The Score wire legend/probability keys stay the SDK's zero-based indexes `"0"` through `"4"` while the criteria describe counts one through five; Choice labels are exact Profile names. The client uses model `jev-latest`, the process `TYPESAFE_API_KEY`, SDK retries and logging disabled, and boundary validation that rejects malformed bodies, wrong or missing answer types, non-finite or out-of-range numbers, missing or unexpected probability keys, distributions that do not sum to one, and foreign Profile options — never trusting SDK types as runtime validation and never repairing output.
- **Deterministic policy.** Code, not Jev, assembles the RouteDecision; Jev supplies judgments, never generated assignment text, and the Router is stateless. A Role's Noul probability `p` answers yes iff `p >= 0.5`, with binary confidence `max(p, 1 - p)`. Every applicable component must meet the whole-decision threshold `0.8`: each Role's Noul confidence, each Role's Score confidence — including a no-Role's Score — and, for yes Roles only, the Choice confidence plus membership of the chosen Profile in that Role's exact candidate list. Only a no-Role's Choice is ignored: its low confidence, absence, or unusable content never vetoes another Role. For each selected Role, assembly emits exactly one Assignment `{profile, count, purpose}` — the chosen Profile; `count = Math.round(score) + 1`, converting the validated raw Score in `[0,4]` to the semantic one-through-five count (nearest level, exact halves up; a raw `0` means one agent; out-of-range values are `invalid_response`, never clamped); and the fixed-template purpose `Perform the ${role} role for the supplied objective.` Assignments sort by Role then Profile ordinal. There is no total-count cap, no top-K Role selection, no confidence multiplication, and no partial RouteDecision — the `0.8` gate is the owner-ratified minimum-component bound, not a calibrated joint probability.
- **Abstain.** Any component below `0.8` or any unavailable or invalid input produces a typed Abstain — `low_confidence` (with the failing question component), `no_assignments` (every Role confidently answered no), `catalog_unavailable`, `invalid_response`, `authentication_unavailable` (missing API key), `transport_failed` (API, connection, timeout, or body-parse failure, carrying at most a bounded HTTP-status component), or `aborted` (caller cancellation, checked before the request and again before a route is returned) — and zero launch effects: no topology, start, prompt, handoff, recipient, or supervision effect occurs. An Abstain carries no default Profile and no partial Assignment list; the manager delegates through explicit launches as before. The wait Reviewer threshold stays `0.5`; the `0.8` gate is Router-only and shares neither threshold nor failure classification with the Reviewer.
- **Decision log.** Exactly one fixed-schema JSONL record is appended per Router outcome — RouteDecision or Abstain — after the decision and before any child mutation, at `.herdr/router/decisions.jsonl` under the trusted manager/session project root, never a caller-controlled child `cwd`. The record carries `timestamp`, the caller `name`, `stateDigest` (SHA-256 of the exact deterministic state JSON supplied to Jev) or an explicit `stateUnavailable` marker when no state was sent, the validated per-question probabilities scoped to the sent question and option keys, and the Assignments or typed Abstain reason/component; persisted purposes must match the deterministic template, and there is no decision ID or stored decision resource. Redaction is by allowlist before `modelSafeJson` runs: no objective/scope/verification text, Profile body or description, source path, tools, environment, key, request/response text, or exception message is persisted. Appends serialize on the existing flock over `decisions.lock` for one short exclusive section — never held across inference or launch — into owner-only directories and a `0600` append-only file; unsafe, symlinked, or foreign-owned targets are refused, a partial trailing line is surfaced rather than repaired, and existing records are never truncated. A failed append is a typed `ROUTER_LOG_UNAVAILABLE` Batch failure with zero launch effects and the decision retained in the failure evidence; recording is never retried into the same broken sink.

#### Batch expansion and execution

- **Expansion.** A RouteDecision expands deterministically in decision order: for each Assignment, `count` children named exactly `{name}-{role}-{N}` with one-based N within the Role across assignments sharing it. A supplied `label` becomes `{label}-{role}-{N}` per child and a `new_tab` `tabLabel` likewise; an absent label leaves the pane labeled with the child name. Profile fallback — same-Role or cross-Role — never renames a child, and child names remain exact targets for `herdr_wait`, `herdr_communicate`, and close. Each derived name must satisfy the same 32-character AgentName contract as an explicit name: overflow is a typed `BATCH_CHILD_NAME_INVALID` child failure reported untruncated, never a shortened or randomized name. Names are claimed against the fresh authoritative snapshot's existing agent names **and** pane labels — a label shadows a name in exact-target resolution, so name-only uniqueness is not sufficient — plus every sibling's derived name; a hit is a typed `BATCH_NAME_COLLISION` child failure. Invalid or colliding children become structured failure entries while unrelated children still expand, and the same fresh check — now covering sibling names **and** sibling labels — re-runs inside each child's lifecycle at the pre-mutation identity check. These snapshot checks are fail-closed evidence, not a cross-process name reservation; authoritative start refusals remain in force. An `existing_pane` placement requires the RouteDecision to expand to exactly one child; anything else is a whole-request `BATCH_PLACEMENT_INVALID` decided before any per-child check or launch effect, and the caller may retry with a multi-child placement.
- **Sequential dispatch.** Children launch strictly one at a time through the shared single-child lifecycle — mutating placement and start calls are never raced — while already-launched children work concurrently; sequential start is not a Fan-out cap. Each child re-acquires the launch gate, re-checks freeze and caller abort, and re-runs its identity checks on a fresh authoritative snapshot, while the single catalog snapshot that fed the Router supplies each child's Profile resolution and the existing runtime, resource, and readiness revalidation stays in force. A child's typed assignment keeps `scope` and `verification` byte-for-byte and derives `objective` as the caller's objective followed by the deterministic purpose and `Instance ${ordinal} of ${count} for this role.`; every renderer, provenance, handoff, attachment, supervision, prompt-confirmation, redaction, and fallback check applies unchanged. The Router-chosen Profile heads only its own existing fallback chain under the same four-attempt reachable-graph bound; no Router default or sibling chain is ever substituted, so an AGY selection simply walks the same per-child lifecycle under provisional supervision that strengthens to exact native-session supervision before success.
- **Best-effort outcomes.** A Batch result is typed `launch_batch` with `outcome` one of `abstained | launched | partial | failed`, the Router result, and one child entry per expanded child — exact `name`, `role`, requested `profile`, `ordinal`, and `status` — where `launched` carries the complete single-launch `LaunchDetails` evidence, `failed` carries the complete redacted structured failure evidence, and `not_started` carries a bounded code. A caller abort stops dispatch, retains completed and in-flight evidence, and marks every undispatched child `not_started`/`ABORTED`; there is no close, rollback, resubmission, or retry of uncertain effects, and a whole Batch is never retried after partial effects. `launched` means every expanded child confirmed its launch, `failed` means no child confirmed a launch — including log, placement, expansion, or all-child dispatch failures — `partial` retains mixed outcomes without rollback, `abstained` is the explicit zero-effect result with `children: []`, and a RouteDecision that expands to no children is `failed` with `BATCH_ROUTE_EMPTY`. Batch outcomes are structured results rather than thrown errors: request validation and the pre-routing freeze gate still fail through the typed error mechanism, while every outcome decided after routing begins carries its evidence inside the result.
- **Result rendering.** The result content leads with a compact all-child manifest — a head line carrying the outcome, the Router kind with its Abstain reason or Assignment count, the child count, and any whole-Batch failure code, then one line per expanded child with its exact name, requested and (when different) selected Profile, outcome, failure code, and pane ID plus supervisor job ID where known — before the verbose details, so the host byte bound can never silently masquerade as a smaller Batch. Structured details keep the full per-child evidence and degrade through the existing explicit truncation envelope (`truncated: true`) at the 60,000-byte MCP result bound; no cap, fabricated success, dropped tail child, or new stored resource is introduced to fit.
- **Caller policy.** ADR-030 is unchanged: `herdr_launch` stays unscoped for every caller, and the `herdr_communicate` leaf restriction is not a Router authorization gate. Orchestrator provenance is stamped on the Profile actually selected by the existing launcher, exactly as in explicit mode.
...
- Retention lasts only as long as the endpoint-private state namespace. Same-UID agents are cooperative rather than hostile isolation, and no fallback can be guaranteed if every monitor dies before persistence. No database, daemon, watcher, transcript scraping, caller path, or per-profile static instruction is introduced.
...
- `ROUTER_LOG_UNAVAILABLE`: the Router decision could not be appended to the local decision log; the Batch launches zero children and retains the decision in the failure evidence.
- `BATCH_PLACEMENT_INVALID`: an `existing_pane` Batch placement did not expand to exactly one child; decided before any per-child check or launch effect.
- `BATCH_NAME_COLLISION`: a derived Batch child name was already held by an existing agent name, a shadowing pane label, or a sibling planned child; that child failed while unrelated children were unaffected.
- `BATCH_CHILD_NAME_INVALID`: a derived Batch child name fell outside the 32-character AgentName contract; reported untruncated, never renamed.
- `BATCH_ROUTE_EMPTY`: a RouteDecision expanded to zero children; the Batch failed with zero launch effects.
...
- `herdr_launch` accepts only validated Profiles — caller-named in explicit mode or Router-selected from the same validated catalog in a Batch — with typed overrides permitted only on the explicit variant; raw kind, argv, and env launch fields are rejected on both variants.
- AGY alone accepts provisional pane, terminal, name, and kind continuity before the first prompt. That continuity is not cryptographic attribution. A same-terminal, same-kind replacement or stale pane-scoped report can be misattributed until exact native-session strengthening. This reduced-assurance risk is bounded by the five-second confirmation window and lifecycle advancement requirement. Pi and Claude remain strict before assignment.
...
- the dual explicit/auto launch schema and its presence discriminator, the one `systemOne` call's per-Role questions and Score-to-count conversion, the whole-decision `0.8` gate and every typed Abstain's zero launch effects, exactly one decision-log append before the first child mutation, `ROUTER_LOG_UNAVAILABLE` stopping all children, sequential per-child dispatch with full evidence and exact `{name}-{role}-{N}` names, the 32-character boundary, name and shadowing-label collisions at expansion and immediately before dispatch, single-child-only `existing_pane`, the abort `not_started` tail, no rollback after partial effects, and the complete all-child manifest under host truncation;
...
5. **Launch and topology gate**
   - Acceptance: launch and pane/tab mutations use explicit placement, labels, resource bookkeeping, autonomous exact close, protected ancestors, and authoritative post-state reads; the auto Batch path makes its single Router decision and decision-log append before dispatching children under the contract above.
...
- Every launch requires a unique caller `name`; a named `profile` runs one explicit child, and an omitted `profile` routes through the Router's Batch expansion so every executed child still uses a validated Profile. Pi and Claude bind exact supervision before focus or assignment. AGY requires a self-contained initial prompt, publishes provisional supervision before its single submission, and strengthens to exact native-session supervision before success, recipient registration, or attachment access. Failed launches are never automatically cleaned.
