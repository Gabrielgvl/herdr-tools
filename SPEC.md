...
- AGY argv is fixed to `--model <model> --mode plan --dangerously-skip-permissions`, followed by scope-normalized profile `--add-dir` values and the Herdr-Tools-owned recipient attachment directory. AGY accepts no prompt file, `--agent`, arbitrary argv, arbitrary env, or non-persistent session mode.
