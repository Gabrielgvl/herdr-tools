...
**Read-only handoff recovery.** After a build, the daemon's `herdr_run` `observe` operation — `{action:"observe", runId: RUN_UUID}` over the executor→MCP surface, where `reconcile` is the same tool's intent-closing action; there is no CLI surface — asks the owner-activated daemon for the observation and refuses `DAEMON_UNAVAILABLE` while the daemon is inactive. It accepts no session, owner, or storage arguments: the caller pane is the injected Herdr pane, and its native manager session is resolved from that pane's verified live occupant in the authoritative snapshot, with the store in the endpoint-private namespace. As for every herdr-tools call, same-user processes sharing the Herdr environment and socket are trusted — the binding is the injected pane's verified live session, not the calling process. Recovery succeeds only for the same unique native manager session and exact retained child and artifact evidence. It returns an observation with `observationOnly: true`, `supervisionRestored: false`, `replayed: false`, and `ownershipTransferred: false`. It never invokes launch, prompt delivery, ownership mutation, or supervisor registration. Legacy or malformed provenance, another or duplicate manager session, a changed child, and an unavailable or digest-mismatched artifact are refusals.

### `herdr_launch`

**Purpose:** Launch exactly one caller-authored **Task** per call as supervised Herdr children and return the uniform launch result. The Task's four semantic fields — `objective`, `scope`, `doneWhen`, `constraints` — are the single work contract: the canonical Task feeds deterministic child instruction rendering, Jev evaluation, the launch assignment-budget preflight, and bounded decision evidence, while supervision reservation and review receive the assignment digest — `objective`, `doneWhen`, and `constraints`, never `scope`. Runtime owns identity, topology, delivery, supervision, and evidence. **Specs, categories, assignments, profiles, roles, caller naming, caller placement, caller delivery, overrides, and request-level supervision digests no longer exist** — the strict schema rejects every undeclared field with no compatibility alias. Raw `kind`, `argv`, and `env` launch fields are likewise rejected: the compiled contract, not the caller, supplies runtime configuration.
...
```text
{
  ...
  tier?: QualityTier,         // optional bounded override; utility|economy|standard|strong|frontier|max; omission lets the workload floor decide
...
- **Evaluation.** One `systemOne` request per Task through `TypeSafeSpecClient` (model `jev-latest`; the credential resolves through `resolveTypesafeApiKey` — the `TYPESAFE_API_KEY` environment variable first, then the `typesafe` `api_key` entry in the Pi auth store). The request sends only the semantic Task projection (`objective`, `scope`, `doneWhen`, `constraints`) — never the requested tier, runtime state, operating-point metadata, or catalog projections — and asks exactly three questions: `done_when_verifiable` — the only semantic launch-quality gate, judging whether `doneWhen` contains concrete, falsifiable completion evidence relevant to `objective` and `scope`; `intent` — a closed choice over the seven workload intents whose top answer is used verbatim regardless of confidence (policy revision `adr-037-p3`); and `weakest_sufficient_tier` — a closed choice over the six tiers judged against the tier rubric, ignoring caller tier, cost, quota, availability, and chain contents. A serialized request over 96 KiB abstains `request_too_large` and is never truncated. There are no retries, repairs, or model-listing calls; malformed, unauthenticated, or failed evaluation becomes a typed abstention. A confident bad gate answer rejects the Task; a passing answer records `not_rejected`, never a certification. Resource selection is deterministic, and Jev never ranks operating points.
- **Tier policy.** A quality tier is a starting quality/compute posture (`utility < economy < standard < strong < frontier < max`). Jev's `weakest_sufficient_tier` answer is the workload floor. The caller's `tier` is a bounded override (`adr-037-p5`): omitted, the effective start is the floor; set, it is `max(floor, min(requested, next(floor)))` — at most one tier above the floor and never below it. A recovery starts at least one tier above the prior route, after the cap. The effective ceiling is `max` because the chain continues upward. Evidence records the requested tier (absent when omitted), floor, effective start, and ceiling. Cost/latency classes remain reviewed catalog metadata; the shipped router does not filter points by tier envelope. In the evidence workload profile, `intent` is Jev's top choice, `verifiability` derives from the gate probability, and `workspaceState` is runtime-derived; `mutation`, `scope`, `horizon`, and `ambiguity` are fixed placeholder values, not classifications. `unknown` persists only so historical evidence stays readable — no new low-confidence `unknown` records or `workload:unknown` tabs are produced. There is no classification receipt and no public workload override.
- **Deterministic authority.** Code, not Jev, assembles the decision: reviewed model/reasoning membership, runner compatibility, authorization and account eligibility, capability/resource-pool/permission/execution-policy limits, effective start tier and chain assembly, recovery lineage and exclusions, and dynamic availability filtering. The compiler remains the final security and capability authority. The fallback chain is the catalog-authored `tierChains` order from the effective start tier through `max`, deduplicated — it widens into stronger tiers by construction and carries no fixed attempt cap; the chain length is the bound. The router admits the first available point in chain order — there is no fitness ordering — and availability is **re-probed immediately before every start attempt** across the whole fallback loop; a point that became unavailable is skipped without another Jev request. A chain with no available point abstains `no_candidates_at_tier` — or `transport_failed` when every probed point is local-capacity-limited. On recovery routes every point on the failed provider is excluded. Typed abstentions produce zero launch effects: no topology, start, prompt, handoff, recipient, or supervision effect occurs.
- **Compilation.** The selected point compiles through `compileCandidateContract` into the runner's effective argv-level configuration: the deterministic resource selection intersected with the reviewed pool and never widened — a name outside the pool is `SELECTION_OUTSIDE_POOL`, a point outside the runner's reviewed model set is `CANDIDATE_NOT_REVIEWED`, an empty permit set argv cannot express is `EMPTY_PERMIT_SET` — plus declared dependencies added, incompatible pairs removed, and deny-channel rules derived, each step recorded as a `derivation`. The contract records installed/selected/exposed/permitted/denied sets per pool field and records `deny-coverage` and `ambient-exposure` gaps rather than dropping them. Pi compiles `--tools`/`--extension`/`--skill` selections (with unconditional `--no-skills`, so the exposed set is exact); Claude compiles `--allowed-tools`/`--disallowed-tools`/`--plugin-dir`; AGY and Devin consume no pool fields — their resources are ambient by mechanism. Compile failures surface as typed abstentions, never partial contracts.
- **Decision log.** Exactly one fixed-schema JSONL record per Task decision is appended at `.herdr/router/decisions.jsonl` under the trusted root — never a caller-controlled child `cwd` — after the decision and before any child mutation. The record carries bounded non-secret evidence: requested tier, workload floor, effective start and ceiling, the intent and tier answers with probabilities and confidence, operating-point metadata and deterministic exclusions, the generated chain and selected point, availability and recovery evidence, and catalog/policy revisions (`adr-037-p5` on new records). Caller text, resource bodies, credentials, raw provider responses, and exception messages remain excluded. Appends serialize on the `decisions.lock` flock into owner-only directories and a `0600` append-only file. A failed append is a typed `ROUTER_LOG_UNAVAILABLE` failure result: zero children launch.
