...
**Wake delivery.** Wakes are report-only, best effort, and never retried. Pi delivers through the existing `sendMessage` custom-context path with `deliverAs: "steer"` and `triggerTurn: true`. Claude has no inbound wake delivery: `manager-claude` declares no `runtime.developmentChannels`, so no `--dangerously-load-development-channels` opt-in is emitted and the session raises neither the organization-policy nor the missing-MCP-server startup warning. Claude manager wakes are recovered by `herdr_jobs` polling, which returns pending events by opaque ID and marks exactly those observed. Generic `runtime.developmentChannels` support remains in the parser and the Claude adapter for any profile that opts in.

**Soft receipts.** Every material event carries an opaque `eventId`. `herdr_jobs get` returns the pending unobserved events and marks exactly those observed; `list` and the Pi active-job UI show unobserved counts. Event, transition, reviewer, and reconciliation history are bounded rings that keep the newest entries and report what is missing as explicit truncation counts. `truncatedEvents` is cumulative over ring evictions and entries omitted from a given projection, and `unobservedEvents` separately reports the pending count. A non-zero `truncatedEvents` therefore states only that retained history is incomplete; it carries no information about whether the missing entries had been observed, and must not be read as proof that a receipt was lost. Polling is bounded, not lossless: a manager polls frequently so pending receipts are collected while still retained, and reconciles authoritative current state when the missing history matters. Nothing is resent.
...
- `wait.reviewCadenceMinutes` is shared with the supervision reviewer. `wait.reviewerModel` is wait-only: the supervisor reviewer pins the authenticated `openai-codex/gpt-5.6-sol` at thinking `max` in code, because it judges a child that has been working continuously with no transition to read.
...
│   │   ├── reviewer.ts              # Sol-max supervisor review
