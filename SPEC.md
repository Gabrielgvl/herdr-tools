...
- unique launch names, strict profile schemas, typed overrides, startup-timeout margins, real failure-envelope fallback, AgentInfo correlation, unchanged identity preflight, one-record idle baselines, working/unknown/missing baseline refusal, advanced-sequence plus non-regressed-revision confirmation for every state, unchanged/regressed/missing counters, cross-record lifecycle skew, screen-flag mismatch, replacement/disappearance/contradiction, post-ack abort, exactly one submission, no failed recipient registration, bounded `PROMPT_UNCONFIRMED` evidence, and failed-launch retention;
