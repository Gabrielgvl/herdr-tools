...
**Review cadence.** A child that has been working continuously for `wait.reviewCadenceMinutes` is reviewed by the supervisor reviewer, whose model is the module constant `SUPERVISION_REVIEWER_MODEL` = `typesafe/jev-latest` (`src/supervision/reviewer.ts`) — a code constant, not a setting, and unrelated to `wait.reviewerModel`. Each review issues one `systemOne` call carrying six independent `noul` predicates — `evidence_sufficient`, `making_progress`, `stalled`, `blocked`, `risk`, `appears_complete` — plus one `reason` choice, over bounded compact metadata, the transcript delta since the previous completed review, the authorial `supervisionDigest` (`doneWhen`/`constraints` recorded at reservation), and the previous review's classification and signals. Deterministic classification runs through `reduceSupervisionReview` (`src/reviewer.ts`): the evidence gate first — `P(evidence_sufficient) < SUPERVISION_EVIDENCE_THRESHOLD` (0.60) yields `unknown` before any signal is evaluated — then precedence `risk` (≥ `SUPERVISION_RISK_THRESHOLD` 0.60) → `blocked` (≥ `SUPERVISION_BLOCKED_THRESHOLD` 0.65) → `appears_complete` (≥ `SUPERVISION_APPEARS_COMPLETE_THRESHOLD` 0.70) → `stalled` (≥ `SUPERVISION_STALLED_THRESHOLD` 0.70, raised to `SUPERVISION_STALLED_FIRST_OBSERVATION_THRESHOLD` 0.85 on a child's first review) → `progress` (≥ `SUPERVISION_PROGRESS_THRESHOLD` 0.60), falling through to `unknown` when no signal crosses. A review whose work cycle ends while its transcript read or model call is in flight is abandoned rather than stored or announced. Results store silently; `stalled`, `blocked`, `risk`, `appears_complete`, and `unknown` wake the manager and the supervisor stays active. A reviewer failure enters one visible degraded episode and retries at the next cadence, and the first success afterwards notifies recovery once. The reviewer never starts a Herdr agent and never selects a substitute model. The host model-service seam (`src/supervision/model-service.ts`) is retained in the registry for hosts that still wire it but is not consulted by the Jev reviewer.
...
**Purpose:** Launch managed agents from caller-authored **specs** and return authoritative launch state. A request composes one or more specs; every spec is evaluated by Jev, admitted by deterministic policy plus availability, and compiled into a runner-specific candidate contract; each admitted spec then dispatches `count` children through the shared launch lifecycle below. **Profiles, roles, and `overrides` no longer exist** — the profile matrix, the `profile` request field, role-derived Router fan-out, and typed per-launch overrides were deleted by the ADR-035 migration, and the strict schema rejects them along with every other undeclared field. Raw `kind`, `argv`, and `env` launch fields are likewise rejected: the compiled contract, not the caller, supplies runtime configuration.

**Input:** one strict object (`SpecLaunchParamsSchema` in `src/launch-schema.ts`). Unknown fields are rejected.

```text
{
  name: string,                                 // required; ^[a-z][a-z0-9_-]{0,31}$ — caller-chosen launch base name
  specs: [                                      // required; at least one; labels unique within the request
    {
      label: string,                            // required; kebab-case ^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$, at most 12 characters, unique per request
      instructions: string,                     // required; non-empty, no NUL — the caller's own instruction text
      assignment: {                             // required; exactly these three fields — extras are rejected
        objective: string,                      // non-empty, no NUL
        scope: string,                          // non-empty, no NUL
        verification: string                    // non-empty, no NUL
      },
      category?: string,                        // optional; kebab-case name resolved against the catalog chains
      count?: integer                           // optional; >= 1, default 1 — replica count for this spec
    }
  ],
  placement?:
  ...
  label?: string,                               // base pane label; default: name
  cwd?: string,                                 // launch cwd; default: current Pi cwd; replica worktrees are created under its repository root
  focus?: boolean,                              // default: false
  assignmentDelivery?: "inline" | "attachment", // default: inline
  supervisionDigest: {                          // REQUIRED at request level — authorial done-when/constraints the supervisor's reviewer judges against (ADR-034 amendment)
    doneWhen: string[],                         // 1-8 non-empty strings of at most 240 characters
    constraints: string[]                       // same bound; author ["none"] when unconstrained
  }
}
```

`specs[].label` is the caller's name for the spec and the naming segment of derived child names; the request-level `label` keeps its pane-label meaning — the overload is deliberate. `supervisionDigest` is required on every request and is request-level, never per-spec: a request without done-when conditions is unjudgeable and fails validation. Assignment size has exactly one authority — the UTF-8 byte length of the *rendered* assignment checked against the selected delivery bound at runtime — so no per-field `maxLength` exists.

#### Evaluation and admission

Every spec — including a request carrying exactly one — passes this pipeline before any launch effect. All of it completes before the first topology mutation:

- **Catalog.** `herdr-profiles/catalog.yaml` under the trusted manager/session project root is loaded once per request and supplies everything the caller does not author: named `categories`, each an ordered chain of `{runner, model}` candidates whose length is fail-closed bounded by `maxAttempts` (declared 4); runner entries for `pi`, `claude`, `agy`, and `devin` carrying reviewed `models`, `quota` keys, `defaults` (thinking/effort/mode/permissionMode/timeoutMinutes/sessionPersistence), `plumbing` mechanism facts, and reviewed resource `pools`; the top-level `skills`/`plugins`/`mcp` reviewed sets; and `quotaSources`. A catalog that cannot be loaded or parsed abstains `catalog_unavailable` for every spec; entries are never invented.
- **Evaluation.** One `systemOne` request per spec through `TypeSafeSpecClient` (model `jev-latest`; the credential resolves through `resolveTypesafeApiKey` — the `TYPESAFE_API_KEY` environment variable first, then the `typesafe` `api_key` entry in the Pi auth store). The request carries the spec plus bounded catalog projections and asks: two quality `noul`s — `instructions_adequate` and `assignment_verifiable`; one `category` `choice` — confirm the caller's `category`, or override it only when another category is a clearly better fit; and runner-qualified per-resource `noul`s covering every chain candidate, so a category override or chain fallback never needs a second model call. There are no retries, repairs, or model-listing calls; malformed, unauthenticated, or failed evaluation becomes a typed abstention.
- **Deterministic policy (`routeSpec`).** Code, not Jev, assembles the decision; Jev supplies judgments only. Order is fixed: quality rejection is checked before every abstention — a confident *bad* answer (probability ≤ 1 − `ROUTER_CONFIDENCE_THRESHOLD` at confidence ≥ `ROUTER_CONFIDENCE_THRESHOLD` = 0.8) yields a `rejected` decision with reason `instructions_inadequate` or `assignment_unverifiable`, while a passing gate records `quality: "not_rejected"` — deliberately not a certification. A category judgment below 0.8 abstains `low_confidence` on `category`; a per-resource judgment below 0.8 abstains `low_confidence` naming the field. Every chain candidate is then probed for availability: `known-exhausted` and `local-capacity-limited` candidates are ineligible while `degraded` and `unknown` stay admissible. `resolveChain` selects the first eligible and available candidate in declared order; an empty outcome abstains `no_assignments` — or `transport_failed` on `availability` when every candidate is locally capacity-limited. Typed abstentions — `low_confidence`, `no_assignments`, `catalog_unavailable`, `invalid_response`, `authentication_unavailable`, `transport_failed`, `aborted` — carry the failing component and produce zero launch effects: no topology, start, prompt, handoff, recipient, or supervision effect occurs.
- **Compilation.** The selected candidate compiles through `compileCandidateContract` into the runner's effective argv-level configuration: Jev's resource picks intersected with the reviewed pool and never widened — a name outside the pool is `SELECTION_OUTSIDE_POOL`, a candidate outside the runner's reviewed model set is `CANDIDATE_NOT_REVIEWED`, an empty permit set argv cannot express is `EMPTY_PERMIT_SET` — plus declared dependencies added, incompatible pairs removed, and deny-channel rules derived, each step recorded as a `derivation`. The contract records installed/selected/exposed/permitted/denied sets per pool field and records `deny-coverage` and `ambient-exposure` gaps rather than dropping them. Pi compiles `--tools`/`--extension`/`--skill` selections (with unconditional `--no-skills`, so the exposed set is exact); Claude compiles `--allowed-tools`/`--disallowed-tools`/`--plugin-dir`; AGY and Devin consume no pool fields — their resources are ambient by mechanism. Compile failures surface as typed abstentions, never partial contracts.
- **Bypass receipts.** The router layer supports replayable bypass receipts: `createBypassReceipt` mints one only from an abstention already persisted to the decision log, bound to `{caller, specRevision, policyRevision, launchIdentity}`; `replayBypassReceipt` re-verifies the binding and digest and returns the frozen admitted result without re-running Jev. A receipt minted over a `transport_failed` abstention carries `quality: "not_evaluated"`; any other abstention replays as `not_rejected`. The public request schema carries no `bypass` field — receipts are router machinery bound to recorded abstentions, not caller input.
- **Decision log.** Exactly one fixed-schema JSONL record per spec decision is appended at `.herdr/router/decisions.jsonl` under the trusted root — never a caller-controlled child `cwd` — after the decision and before any child mutation. The record carries the caller, the spec's decision kind, quality outcome, category, selected candidate, availability evidence, and the compiled contract's reviewed fields; caller text, resource bodies, keys, request/response text, and exception messages are redacted by allowlist before serialization. Appends serialize on the `decisions.lock` flock into owner-only directories and a `0600` append-only file. A failed append is a typed `ROUTER_LOG_UNAVAILABLE` failure: zero children launch and the decision is retained in the failure evidence.

#### Catalog pools and ambient resources

The reviewed pools cover only resources intentionally promoted for chain-selectable compilation — Pi `--skill`/`--extension`/`--tools` units, Claude `--plugin-dir` roots, and the MCP servers those plugins provide. The catalog-vs-disk inventory (`.herdr/artifacts/resource-mapping/skills-extensions.md`) reconciles every installed skill and extension against the pools: 41 installed entries are present on disk but absent from the catalog pools — ambient Pi extensions (`herdr-agent-state`, `herdr-question-alert`, `herdr-tools`, `tmux-bg-notify`), ambient `/home/gabriel/.agents/skills` skills, Pi skills installed outside the catalog, and host-provided plugin entries. These are host-provided ambient resources, **not** chain-selectable resources; their absence from pools is by design unless the owner promotes them into the reviewed catalog. The same inventory reports 0 catalog declarations missing on disk — all 47 unique skill declarations and all 8 extension/plugin declarations resolve to installed paths.

#### Child expansion, naming, and replicas

Each admitted spec expands to `count` children:

- Child names are exactly `{request.name}-{spec.label}-{N}` with one-based N within the spec label. A supplied request `label` becomes `{label}-{spec.label}-{N}` per child and a `new_tab` `tabLabel` is suffixed likewise; an absent label leaves the pane labeled with the child name. Candidate fallback along the chain never renames a child, and child names remain exact targets for `herdr_wait`, `herdr_communicate`, and close.
- Every derived name must satisfy the 32-character AgentName contract; overflow is a typed `BATCH_CHILD_NAME_INVALID` per-child failure reported untruncated, never a shortened or randomized name. Names are claimed against the fresh authoritative snapshot's existing agent names **and** pane labels — a label shadows a name in exact-target resolution — plus every sibling's derived name and label; a hit is a typed `BATCH_NAME_COLLISION` per-child failure. Invalid or colliding children become structured failure entries while unrelated children still expand, and the same fresh check re-runs inside each child's lifecycle at the pre-mutation identity check. These snapshot checks are fail-closed evidence, not a cross-process name reservation; authoritative start refusals remain in force.
- An `existing_pane` placement requires the request to expand to exactly one child in total; anything else is a whole-request `BATCH_PLACEMENT_INVALID` decided before any per-child check or launch effect, and the caller may retry with a multi-child placement.
- `count <= 1` is a pass-through: the child launches in the requested `cwd` with no worktree effects. `count > 1` gives each replica an isolated detached-HEAD worktree at `<repo>/.herdr/worktrees/<childName>` with a provenance marker at `.herdr/worktrees/.meta/<childName>.json`; creation serializes on `.herdr/worktrees/creation.lock`, directories are created owner-only and proven trusted, and a same-name stale directory is reclaimed only when its marker proves ownership. Replica worktrees are ephemeral writers — the spec baseline requires leaving deliverable work uncommitted.
- Children dispatch strictly one at a time through the shared lifecycle — mutating placement and start calls are never raced — while already-launched children work concurrently; sequential start is not a fan-out cap. Each child re-acquires the launch gate, re-checks freeze and caller abort, and re-runs its identity checks on a fresh authoritative snapshot.
...
- Fallback stays inside `agent_start`, before supervision binding and before assignment. The admitted category chain's remaining admissible candidates are tried in declared order under the catalog's `maxAttempts` bound, each under its own compiled contract — fallback can change the runner and model that actually start the child, and it reuses the resolved pane. An exhausted chain stops and reports bounded attempt evidence; no candidate is improvised. There is no post-spawn fallback.
- Launch details report the spec label and category, the requested and selected candidates, effective runtime/model/timeout/permissions, bounded attempt evidence, readiness timing/records, authoritative IDs/post-state, prompt submission/observation evidence, and any visible provenance. They include independent monotonic safe-integer durations for selected start through readiness, immediately before stdin submission through completion of typed acknowledgement parsing and identity validation, and validated acknowledgement through semantic confirmation; observed durations are never clamped to their configured maxima. Readiness diagnostics use a four-record fixed-whitelist projector: only fixed identity/lifecycle keys, bounded primitive values and malformed/missing markers, plus the four fixed agent-session fields survive. Arbitrary keys, nested trees, arrays, prototype-shaped properties, and oversized values cannot enter readiness records or readiness error metadata. Failed launches retain created resources and never auto-clean them. When a typed CLI failure reaches the launch boundary, argv calls retain bounded fixed-shape stdout/stderr evidence; stdin deliveries retain only non-textual stream presence/size/truncation. Raw stdin stdout/stderr and prompt bodies are never retained or published.

#### Results and public projections

- A request whose admitted specs expand to exactly one child returns the single-launch `launch` result with complete `LaunchDetails` evidence. Any other request returns a `launch_batch` result carrying `outcome` one of `abstained | launched | partial | failed`, the per-spec router decisions, and one entry per expanded child — exact `name`, `spec.label`, `ordinal`, and `status` — where `launched` carries the complete single-launch evidence, `failed` carries the complete redacted structured failure evidence, and `not_started` carries a bounded code. `abstained` means no spec was admitted and is the explicit zero-effect result with `children: []`; `launched` means every expanded child confirmed its launch; `failed` means no child confirmed a launch; `partial` retains mixed outcomes without rollback. A caller abort stops dispatch, retains completed and in-flight evidence, and marks every undispatched child `not_started`/`ABORTED`; there is no close, rollback, resubmission, or retry of uncertain effects, and a request is never retried as a whole after partial effects. Batch outcomes are structured results rather than thrown errors: request validation and the pre-routing freeze gate still fail through the typed error mechanism, while every outcome decided after routing begins carries its evidence inside the result.
- The result content leads with a compact all-child manifest — a head line carrying the outcome, the per-spec decision kinds with abstain reasons, the child count, and any whole-request failure code, then one line per expanded child with its exact name, spec label, candidate, outcome, failure code, and pane ID plus supervisor job ID where known — before the verbose details, so the host byte bound can never silently masquerade as a smaller batch. Structured details keep the full per-child evidence and degrade through the existing explicit truncation envelope (`truncated: true`) at the 60,000-byte MCP result bound; no cap, fabricated success, dropped tail child, or new stored resource is introduced to fit.
- **Public job projection (consumer-visible B9 rename).** The `herdr_jobs` supervision view exposes `candidateName` — the model of the candidate that actually bound and started the child — under `request.child`, `supervision.child`, and `supervision.provisional`. When chain fallback changed which candidate started the child, `requestedCandidateName` carries the candidate recorded at reservation; it is omitted when the two are equal. Consumers that read the retired `profileName`/`requestedProfileName` fields must update.
- **Caller policy.** ADR-030 is unchanged: `herdr_launch` stays unscoped for every caller, and the `herdr_communicate` leaf restriction is not an admission gate. Orchestrator provenance is stamped from the bound candidate the launcher actually started.
...
- `ROUTER_LOG_UNAVAILABLE`: a spec decision could not be appended to the local decision log; the request launches zero children and retains the decision in the failure evidence.
- `BATCH_PLACEMENT_INVALID`: an `existing_pane` placement did not expand to exactly one child; decided before any per-child check or launch effect.
- `BATCH_NAME_COLLISION`: a derived child name was already held by an existing agent name, a shadowing pane label, or a sibling planned child; that child failed while unrelated children were unaffected.
- `BATCH_CHILD_NAME_INVALID`: a derived child name fell outside the 32-character AgentName contract; reported untruncated, never renamed.
- `BATCH_ROUTE_EMPTY`: a request expanded to zero children; retained in the failure-classification table — spec expansion reports per-child failures instead.
...
- `herdr_launch` accepts only the strict spec request; every admitted child runs a candidate contract compiled from the reviewed catalog pools — Jev may narrow a pool, never widen it — and raw kind, argv, env, `profile`, and `overrides` fields are rejected at validation.
...
5. **Launch and topology gate**
   - Acceptance: launch and pane/tab mutations use explicit placement, labels, resource bookkeeping, autonomous exact close, protected ancestors, and authoritative post-state reads; a multi-spec request appends its per-spec decisions to the decision log before dispatching children under the contract above.
...
- Every launch requires a unique caller `name`; caller-authored specs route through Jev evaluation and deterministic admission so every executed child runs a contract compiled from the reviewed catalog. Pi and Claude bind exact supervision before focus or assignment. AGY requires a self-contained initial prompt, publishes provisional supervision before its single submission, and strengthens to exact native-session supervision before success, recipient registration, or attachment access. Failed launches are never automatically cleaned.
