...
- Profile body sources are created before topology mutation. Initial prompts are sent only after the selected agent starts, wrapped in the mandatory v1 assignment envelope, and verified as `working`. The launch wait uses a 10,000 ms CLI deadline so Herdr's fixed 5,000 ms state-change observation can return the exact typed `agent_prompt_stalled` response. For that response, the existing new-pane recovery remains available; an existing-pane recovery may send at most one Enter only when the pane is owned by this runtime, was agent-free in the authoritative pre-launch snapshot, and a fresh authoritative agent read/get proves the exact started name, pane, kind, idle state, and stalled state-change sequence. Unowned, preexisting-agent, mismatched, missing, or otherwise uncertain existing-pane evidence fails closed without sending keys.
