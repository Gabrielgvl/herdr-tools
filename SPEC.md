...
Every successful `herdr_launch` creates a live supervisor for the exact child it launched, including a launch with no `initialPrompt` and a launch into an existing pane. Supervision binds as soon as readiness proves the exact child identity, before optional focus or assignment. Assignment confirmation remains a separate launch-success gate. Supervision is observation and notification only: it never mutates the child, never gates its work, and never becomes a public tool.

**Reserve, then bind.** `herdr_launch` reserves supervision before its first topology mutation, in phase `supervision_reserve`; a failure there refuses the launch with `SUPERVISION_UNAVAILABLE` and `effectCertainty: "absent"`. Phase `supervision_bind` follows readiness and precedes focus and prompt dispatch. Binding validates a fresh authoritative snapshot, drains queued evidence while the public view remains `reserved`, and commits the selected profile, selected kind, and `request.targetIds: [exactPaneId]` before publishing bound `active` or `degraded` state. A queued closure, release, replacement, identity loss, or failed commit makes bind reject as `SUPERVISION_UNCONFIRMED`; it rolls back provisional request fields to `targetIds: []`, releases only the unbound reservation, and performs no focus, prompt, recipient registration, retry, or child cleanup. The child and binding evidence remain available for inspection. Profile fallback stays strictly inside `agent_start`, before assignment and before binding. A successful launch returns the stable supervisor job ID in both its model-visible content and its details.

After bind succeeds, a later focus, prompt transport, acknowledgement, confirmation, recipient, or rendering failure never releases or cancels the supervisor. An acknowledged prompt whose consumption is not proven throws `LAUNCH_FAILED` with `causeCode: "PROMPT_UNCONFIRMED"`, `assignmentState: "unconfirmed"`, `promptSubmitted: true`, the exact pane ID, the retained active supervisor job ID, and fixed bounded recovery guidance. The launch never auto-sends Enter, retries assignment or start, closes or reuses the pane, registers a recipient, or authorizes dependent work. Managers inspect the existing child with `herdr_inspect` and the supervisor with `herdr_jobs get`.
...
- Launch requires a unique caller name and named profile, binds supervision immediately after exact readiness before focus or assignment, verifies initial work, streams progress, and never cleans failed launches.
