...
3. ~~`operation:"prompt"` remains distinct: it rejects a `working` target with `TARGET_BUSY` rather than intentionally steering it.~~ **Reversed 2026-09-22:** `prompt` on a `working` target submits through the identical `agent prompt` path as `steer` — the wire is one bracketed-paste write either way, and the target runtime decides how it lands (Pi steers mid-turn, Devin queues for the composer flush). The distinction that remains is declared intent: the envelope `kind` and `route` still record `prompt`, and `preState` records that delivery occurred while working. `blocked` stays refused (`TARGET_BLOCKED`) — a pane awaiting input could consume the text as its answer; explicit `steer` remains the deliberate escape hatch. The driver was pane→manager reporting: workers sending `kind:"result"` to a manager that is normally `working` failed closed, forcing callers to know the steer escape hatch for the common case.
