...
3. ~~`operation:"prompt"` remains distinct: it rejects a `working` target with `TARGET_BUSY` rather than intentionally steering it.~~ **Reversed 2026-09-22:** `prompt` submits through the identical `agent prompt` path as `steer` on every sendable state — the wire is one bracketed-paste write either way, and the target runtime and server decide how it lands (Pi steers mid-turn, Devin queues for the composer flush; a server-side `agent_blocked` rejection still surfaces as `TARGET_BLOCKED`). The prompt/steer distinction is declared intent only: the envelope `kind` and `route` record `prompt`, and `preState` records the state the delivery landed in. State gating added nothing — explicit `steer` already permitted every delivery the prompt gate refused, and on `blocked` the server itself is the rejection authority. The driver was pane→manager reporting: workers sending `kind:"result"` to a manager that is normally `working` (or `blocked` awaiting input the result may be) failed closed, forcing callers to know an escape hatch some tool-schema renderings do not even show.
