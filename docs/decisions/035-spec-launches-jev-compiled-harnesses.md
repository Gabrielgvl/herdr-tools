...
- **The profile model is deleted.** No role files, no `{role}-{runner}` profiles, no `profile` launch parameter. The unit of launch is a caller-authored **spec**: `{label, instructions, assignment{objective, scope, verification}, category?, count?}`.
