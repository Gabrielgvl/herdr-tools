...
- [ ] Add `claude-manager-plugin/` with the manifest, top-level server map, and `herdr-manager` skill.
  ...
  - Verify: load the package with `--plugin-dir` in a disposable manager session; `/mcp` lists the `herdr` server with seven tools under the expected prefix; the skill is discoverable.
