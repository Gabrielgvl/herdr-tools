...
- Exact-identity, revision-gap, health, and recovery behavior for periodic reconciliation.
- Binding a supervisor job request's `targetIds` to the exact pane when identity binding succeeds.
...
The current source has six obsolete behaviors:
...
4. `src/supervision/registry.ts` registers supervisor requests with `targetIds: []` and binding updates only profile and kind, so an active job does not identify its exact pane in the request snapshot.
5. `src/tools/wait.ts` constructs a wait reviewer for every long wait and sends every target to that reviewer, even when an active supervisor already owns semantic review for the same exact child.
6. A wait reviewer classification of `unknown` contributes to `manager_judgment_required` even when a fresh authoritative observation still reports that target as `working`.
...
A bound supervisor may be `active` or visibly `degraded`. Degradation does not transfer semantic review ownership back to `herdr_wait`. The supervisor already reports degradation and retries under its own contract. Starting a hidden low-thinking fallback reviewer would conceal that state and recreate duplicate ownership.
...
A reserved supervisor has no pane identity and may temporarily carry `request.targetIds: []`. Successful binding atomically replaces that value with the one exact bind pane ID before the job can be observed as bound and active. A bound supervisor request never retains the empty array.
...
4. Bind the supervisor in `supervision_bind` immediately after readiness returns the exact captured identity and lifecycle anchor. The same binding operation publishes `request.targetIds: [capturedIdentity.paneId]` with the selected profile and kind.
5. Apply optional focus only after binding succeeds.
...
- The existing default socket request bound of 10,000 ms applies to each periodic snapshot.
- At most one periodic snapshot is in flight. A slow or failed read never overlaps the next read.
...
With an available socket service, a transition omitted from the event stream is observed by a snapshot that starts within 30 seconds and completes within the 10-second request bound. The default maximum stale-status window is therefore 40 seconds. This is a correctness bound, not a service-level promise when the authoritative snapshot service itself is unavailable.
...
A socket event that arrives around the same time as a periodic snapshot can be covered first by either source. The pane revision watermark remains the only deduplication authority. Once the snapshot adopts a revision, a later event at or below that revision is ignored.

### 7.3 Exact identity and revision rules

Every live supervisor evaluates the shared snapshot against its own complete bound identity.

- No unique pane record means identity cannot be proven. The supervisor settles under the existing pane-closed or identity-lost rule supported by the snapshot evidence.
- A different terminal, name, kind, or complete session settles `identity_replaced`.
- An agent-free but otherwise continuous pane settles `released` under the existing contract.
- A continuous occupant with revision equal to the watermark and status equal to the projected status confirms currency and produces no transition or gap.
- A continuous occupant with revision equal to the watermark but different status violates the expected revision invariant. The supervisor emits one high-priority `evidence_gap` with reason `status_changed_without_revision`, adopts the authoritative status at the same watermark, and records the snapshot transition. It does not remain stale merely because the revision invariant failed.
- A continuous occupant with revision greater than the watermark proves that the supervisor did not fold all changes that reached authoritative state. The supervisor emits exactly one high-priority `evidence_gap` for that observed jump, adopts the snapshot revision and status, and records any status transition with source `snapshot`.
...
A revision jump emits `evidence_gap` even when the endpoint status equals the prior status. A child may have changed state and returned between snapshots, so equal endpoint status does not prove that no lifecycle transition was missed.

When adopting a changed endpoint status, existing material transition rules still apply. For example, a periodic snapshot that corrects `working` to `done` records the snapshot transition and the existing work-cycle event in addition to the gap. Later replay or stream events at the adopted revision cannot duplicate either transition.
...
The first failed read or contradictory lower revision in an episode emits one `reconciliation_degraded` material event and manager wake. Further failures increment the bounded counter and update progress without duplicate wakes. The first later successful exact-identity reconciliation emits one `reconciliation_recovered` event, resets the counter, and updates `lastSuccessAtMs`.
...
The binding update that records the selected kind and profile also records the exact pane:

```ts
request.targetIds = [binding.identity.paneId];
```

The update occurs before `SupervisionReservation.bind(...)` resolves and before the supervisor view reports `state: "active"`. `targets`, `targetIds`, and `target_generation_refs` remain one-item aligned arrays for a bound supervisor. `herdr_jobs get`, list summaries, Pi active-job views, and model projections therefore stop reporting an empty active target ID.

The active-supervisor coverage query in section 8 uses the supervisor's live private identity, not `request.targetIds`. The request field is a bounded public bind record and cannot weaken exact matching.
...
### 9.3 Model-visible wait progress

The latest wait progress details add a bounded ownership projection after each review decision:
...
A reviewer result is advisory until Tools performs the existing authoritative post-review refresh needed for settlement precedence.
...
- `unknown` is stored in reviewer summaries and progress, but it does not require manager judgment when the fresh authoritative target state is `working`.
- `unknown` still requires manager judgment when the target cannot be mapped exactly or the fresh authoritative state is `unknown`, missing, `idle`, `blocked`, or `done` and the requested condition remains unmet.

If an `unknown` review is the only attention classification and every corresponding target is authoritatively `working`, the wait continues. A later authoritative condition match, timeout, target error, stronger reviewer classification, or cancellation settles it under existing precedence.
...
- Long waits still evaluate their conditions. Active supervisors own semantic review for their exact children, while unsupervised targets retain the explicit wait reviewer.
- Supervisor reviewer findings and degradation remain on supervisor jobs and wakes. They are not copied into wait results.
- A wait reviewer `unknown` does not override fresh authoritative `working` state by itself.
...
- Full identity and session fields do not enter wait progress coverage, reconciliation health, gap details, or launch diagnostics.
...
- **AC-S1:** With one or more live supervisors, the monitor performs exactly one shared periodic `session.snapshot` attempt every 30,000 ms and never one attempt per child.
- **AC-S2:** No periodic snapshot overlaps another. The loop stops with no observers and on session shutdown, and it leaves no referenced timer.
...
- **AC-S4:** A continuous occupant at the same revision and same status produces no gap or transition. A same-revision status mismatch emits one `evidence_gap` and adopts the authoritative status. A higher revision emits exactly one `evidence_gap`, advances the watermark, and adopts authoritative status even when endpoint status is unchanged.
- **AC-S5:** A missed `idle` to `working` socket transition is corrected by the next successful periodic snapshot, records source `snapshot`, and cannot remain stale beyond 40 seconds under the fixed 30-second interval and default 10-second request bound.
- **AC-S6:** Missing, released, and replaced occupants settle only under the existing exact-identity rules. A lower revision is not adopted and enters visible reconciliation degradation.
- **AC-S7:** Snapshot failure emits one `reconciliation_degraded` event per episode, retries without overlap, exposes bounded health, and emits one `reconciliation_recovered` event on the first later success.
- **AC-S8:** A reconciliation-degraded but live supervisor remains the semantic review owner and is visibly `degraded`. No CLI or reviewer fallback starts.
- **AC-S9:** Successful bind atomically publishes `request.targetIds: [exactPaneId]` with selected kind and profile before active state is observable. A reserved job may be empty, but a bound active job may not.
- **AC-S10:** `targets`, `targetIds`, and `target_generation_refs` remain aligned one-item arrays, and public job projections expose the exact bind pane without exposing full identity.

### Launch and supervision

- **AC-L1:** Supervision reservation still completes before the first topology mutation.
- **AC-L2:** After exact readiness, `reservation.bind(...)` is called exactly once before optional focus and before any `agent prompt` dispatch.
- **AC-L3:** A bind failure sends no focus or prompt mutation, registers no recipient, performs no retry or child cleanup, and releases only the unbound reservation.
...
- **AC-W7:** `semanticReview.supervisorCovered` and `semanticReview.explicitReviewerTargetIds` appear in bounded wait progress after each cadence decision.
- **AC-W8:** A sole `unknown` review for an authoritatively working target is retained but does not settle the wait. The wait can later meet its condition or time out.
- **AC-W9:** `stalled`, `blocked`, `risk`, and a qualifying non-working or unmapped `unknown` retain manager-judgment behavior after authoritative condition and timeout precedence.
...
- **AC-D3:** Tests explicitly prove no duplicate prompt, no launch cleanup, no secret leakage, no indefinitely stale status after a missed transition while snapshots succeed, no empty target ID on a bound supervisor, and no post-bind identity-proven child losing supervision because launch failed.
...
- Release only when reservation never bound.
...
- A supervisor anchored `idle` with no socket event whose next authoritative snapshot reports the exact child `working` at a higher revision. The test must prove one gap, one snapshot transition, adopted working status, and no stale idle projection.
- A higher revision with the same endpoint status still produces one gap.
- Same revision with the same status stays silent, while same revision with a changed status emits a gap and adopts the status.
- Lower revision degradation, exact replacement, release, pane absence, snapshot timeout, repeated failure, and one recovery event.
- Bounded reconciliation timestamps and counters in `herdr_jobs get`.
- Atomic bind publication of selected profile, selected kind, and `targetIds: [paneId]`, with no observable bound active state carrying `targetIds: []`.
- One-item alignment of target names, IDs, and generation references.
...
- [ ] **Slice 1: Add the one session-level periodic snapshot loop**
  ...
  - Acceptance: AC-S1, AC-S2, and the callback episode portion of AC-S7.
  - Verify: `npx vitest run --coverage.enabled=false test/unit/supervision-monitor.test.ts test/unit/supervision-events.test.ts`

- [ ] **Slice 2: Reconcile every supervisor from periodic snapshots**
  - Files: `src/supervision/supervisor.ts`, `src/supervision/state.ts`, `test/unit/supervisor.test.ts`
  - Acceptance: AC-S3 through AC-S8.
  - Verify: `npx vitest run --coverage.enabled=false test/unit/supervisor.test.ts`

- [ ] **Slice 3: Bind exact job target IDs and publish reconciliation health**
  - Files: `src/supervision/registry.ts`, `src/job-registry.ts`, `test/unit/supervision-registry.test.ts`, `test/unit/supervision-projection.test.ts`
  - Acceptance: AC-S9 and AC-S10, including bounded public reconciliation health.
...
- [ ] **Slice 4: Expose exact active-supervisor coverage**
  - Files: `src/supervision/state.ts`, `src/supervision/supervisor.ts`, `src/job-registry.ts`, `test/unit/supervisor.test.ts`, `test/unit/supervision-registry.test.ts`
  - Acceptance: AC-W1 and AC-W2.
  - Verify: `npx vitest run --coverage.enabled=false test/unit/supervisor.test.ts test/unit/supervision-registry.test.ts`

- [ ] **Slice 5: Partition wait semantic review**
  - Files: `src/tools/wait.ts`, `test/unit/wait.test.ts`
  - Acceptance: AC-W3 through AC-W10.
  - Verify: `npx vitest run --coverage.enabled=false test/unit/wait.test.ts`
...
- [ ] **Slice 6: Bind launch supervision at identity proof**
  ...
  - Acceptance: AC-L1 through AC-L6, AC-L11, and AC-L12.
...
ADR-019 made snapshots strictly on demand after a redundant per-launch read disturbed a clientless server. Stream-only observation has now produced a stale active supervisor, so zero periodic reads is no longer acceptable. One shared snapshot every 30 seconds adds constant session-level load rather than load proportional to child count. A shorter interval would reduce staleness but raise socket pressure. A longer interval could let stale state survive into the minimum one-minute review cadence.

### Revision gaps are deliberately conservative

Pane revision can advance for output as well as lifecycle status. A periodic snapshot can therefore emit `evidence_gap` when the endpoint status did not change. Suppressing that gap would falsely claim that no hidden lifecycle transition occurred and returned to the same state. The design prefers a bounded visible false positive over silent stale evidence.

### Reconciliation failure degrades rather than falls back

A failed periodic snapshot does not close a healthy event subscription and does not start CLI polling. It marks reconciliation health degraded, retries on the next fixed interval, and preserves the last proven status with explicit timestamps. Current status can remain stale while the authoritative service is unavailable, but it cannot remain silently stale.

### Earlier binding adds no new authority

Binding moves earlier, but it still uses the same exact readiness identity and the supervisor's authoritative bind snapshot. The change does not let launch infer supervision from a start response. The cost is that a supervisor can exist for a launch that later reports assignment failure. That retained job is the intended safety record.
...
A reviewer that cannot classify an authoritatively working child no longer ends the wait by itself. The wait may continue to its timeout. This favors fresh authoritative lifecycle state over model uncertainty and avoids false manager-judgment settlement.
...
- Make every unobserved revision advance gap-visible and adopt its authoritative endpoint.
- Publish the exact bind pane in a bound supervisor request's `targetIds`.
- Bind before any post-readiness mutation.
...
- Publish a bound active supervisor with empty `targetIds`.
- Treat reservation as active coverage.
