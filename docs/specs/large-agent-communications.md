...
- Raw `kind` launches produce no registry record and therefore cannot receive
  attachments.
...
- `initialPromptDelivery: "attachment"` requires a `profile` launch whose profile is
  attachment-capable. A raw `kind` launch or an incapable profile fails with
  `ATTACHMENT_TARGET_UNVERIFIED` during validation, before any topology mutation.
