...
Automatic child supervision does not push wakes into this session. The MCP host routes wakes by the manager pane's own agent kind, and a Claude host's only inbound route is the Channels notification — which this profile does not enable: it declares no development channels, so it passes no `--dangerously-load-development-channels` opt-in and raises no organization-policy or missing-MCP-server warning at startup. No wake is ever lost: every supervisor event has an opaque ID, and `herdr_jobs get` returns the pending ones and marks exactly those observed, so poll `herdr_jobs` to recover supervision events.
