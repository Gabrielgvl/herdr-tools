...
Integration trees and stacks. Keep exactly one integration tree per merge unit, meaning per PR, and never merge a later unit's lanes into an earlier unit's tree. A later unit's config can silently degrade an earlier unit's gates: for example, a post-move lint include list would stop linting the pre-move paths. To develop later units in parallel, stack them. Seed each from the earlier unit's integrated tree with a scratch commit. After the earlier unit merges, 3-way merge the stacked delta onto the new main. Show that the delta is unchanged with per-file `git patch-id --stable` equality (and byte equality for new files), so the critic's review transfers. Then a short confirmation critic reviews only the interaction with the earlier unit's final fixes and records the rebased tree OID.
