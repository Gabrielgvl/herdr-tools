...
Never drive Herdr through Bash, a shell wrapper, or another tool. If typed Herdr capability is unavailable or incompatible, raw `herdr` CLI fallback through the host's owner-gated shell tool is permitted only when the owner directly authorizes it for the current task. Every authorized raw-CLI fallback must preserve that envelope: exact stable targets, authoritative inspection, provenance-wrapped sends, and bounded waits. Raw agent start cannot mint a typed supervisor job: disclose missing typed auto-supervisor coverage, use only available native bounded wait/review support, and never claim equivalence with typed supervision. Never delegate Herdr work to a native subagent or the `Task` tool. If a server or capability is unavailable and no authorized fallback exists, stop and report the exact blocker; do not substitute a weaker mechanism.

Inspect authoritative state before acting. Targets are exact opaque IDs, `current`, exact pane labels, or unique exact agent names. There is no fuzzy, prefix, or focused-pane fallback. Launch Tasks only; the runtime places every child on the classified `workload:<intent>` tab — never the isolated manager/caller tab — as an unfocused right-split pane. Send prompts and launches through `herdr_communicate` and `herdr_launch`, which attach the mandatory provenance envelope. `herdr_communicate` also has strict `cancel` and `interrupt` variants with exactly `{target,operation}` and no extra fields. Use them only for an exact authoritative `working` target; the tool binds pane ID, terminal ID, and the full agent-session identity across a snapshot and fresh `agent get`, sends exactly one `esc` for cancel or one `ctrl+c` for interrupt, and performs its own fixed 5-second wait plus independent final snapshot. Do not retry, add escalation keys, infer causality, or treat a disappearing pane as a successful cancel. `agent_exited` is only the tool's narrow post-dispatch absence proof.
...
Assignment confirmation remains a separate gate. If an acknowledged initial prompt is not semantically confirmed, the launch child reports `state: "failed"` with `error.code: "PROMPT_UNCONFIRMED"` and the retained active supervisor job ID; resolve the exact pane ID from that supervisor job. Record both IDs, inspect the child with `herdr_inspect`, and inspect the supervisor with `herdr_jobs get`. Do not relaunch, resend the Task, auto-send Enter, close or reuse the pane, register a recipient, or continue dependent success assertions. The supervisor remains responsible for lifecycle observation. `PROMPT_UNCONFIRMED` means not proven, possibly consumed.
...
Close only panes and tabs this session created and still owns. Never close the owner's pane, the manager's own pane, or a resource another session owns. Use authoritative post-state after mutations and report uncertainty rather than guessing. Hand off before context exhaustion, including exact IDs, operating-point identity, evidence, blockers, and cleanup state.

When this skill is loaded by `manager-claude`, no supervisor wake is pushed into the session. The profile declares no development channels, so nothing is delivered inbound and no organization-policy or missing-MCP-server warning is raised at startup. (Devin- and Pi-hosted managers do receive inbound wakes as `kind: supervision` / `kind: wait` envelopes; a Claude host's only inbound route is Channels, which this profile does not enable.) Silence is never evidence that children are idle: recover supervision events by polling `herdr_jobs` for unobserved counts. That includes a child's result. Its steer can be refused (`TARGET_BLOCKED`) while the manager pane is busy, so when a supervisor shows the child `done` with an `accepted` handoff and no result has arrived, read the handoff file and continue. Don't wait for a resend. An open owner question or menu in the manager pane is the most common cause of those refusals, so right after every owner interaction, poll every supervisor and read any accepted handoffs before doing anything else.
