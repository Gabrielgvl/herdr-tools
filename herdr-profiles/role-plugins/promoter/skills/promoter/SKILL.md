...
If the assignment supplies an exact handoff path, `edit` and `write` may persist only that handoff. Prepare the PR and communication package, but never execute an external effect from an agent-authored promoter assignment. Return the verified local commit and drafts to the originating owner-authorized session for any authorized push, PR creation, message, publication, deployment, or spend.
