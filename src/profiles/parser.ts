...
function parseRuntime(value: unknown): RuntimeProfile {
  ...
  if (value.kind === "pi") {
    exactKeys(value, ["kind", "model", "thinking"], "runtime");
    if (!THINKING_LEVELS.includes(value.thinking as ThinkingLevel)) fail("runtime.thinking is invalid");
    return { kind: "pi", model: stringField(value.model, "runtime.model"), thinking: value.thinking as ThinkingLevel };
  }
  exactKeys(value, ["kind", "model", "effort"], "runtime");
  if (!CLAUDE_EFFORTS.includes(value.effort as ClaudeEffort)) fail("runtime.effort is invalid");
  return { kind: "claude", model: stringField(value.model, "runtime.model"), effort: value.effort as ClaudeEffort };
}
