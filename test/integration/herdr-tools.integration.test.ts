...
describe.skipIf(!enabled)("disposable Herdr integration", () => {
  ...
  it("refuses attachment delivery to unprofiled targets without publishing or sending", async () => {
    const rawLaunch = await tool("herdr_launch").execute("launch-raw", { name: "integration-raw-worker", kind: "pi", placement: { mode: "new_tab", tabLabel: "raw-launch" } }, signal(), undefined, toolContext());
    const rawPaneId = String(resultObject(rawLaunch.details).paneId);
    const promptsBefore = state.stdinCalls.length;
    await expect(tool("herdr_communicate").execute("refuse-raw-attachment", { target: rawPaneId, operation: "prompt", text: "unusable reference", delivery: "attachment" }, signal(), undefined, toolContext()))
      .rejects.toMatchObject({ code: "ATTACHMENT_TARGET_UNVERIFIED", details: { delivery: "attachment", phase: "verify_recipient" } });
    await expect(tool("herdr_launch").execute("refuse-raw-launch", { name: "integration-raw-attach", kind: "pi", initialPrompt: "unusable reference", initialPromptDelivery: "attachment" }, signal(), undefined, toolContext()))
      .rejects.toMatchObject({ code: "ATTACHMENT_TARGET_UNVERIFIED", details: { delivery: "attachment", phase: "validate" } });
    expect(state.stdinCalls).toHaveLength(promptsBefore);
  }, 180_000);
  ...
  it("routes wrapped text over the session-bound stdin transport and publishes exact artifacts", async () => {
    ...
    expect(state.profilePromptContent).toContain("Implement the assigned change");
