...
function installedView(request: SupervisorJobRequestSnapshot): SupervisionJobView {
  return {
    ...
    truncatedEvents: 0,
    unobservedEvents: 0,
    child: { agentName: request.child.agentName, agentKind: request.child.agentKind, paneId: "p1", terminalId: "t1", operatingPointId: request.child.operatingPointId },
    status: "working",
  };
}
...
describe("coverage contract edges", () => {
  ...
  it("covers every supervised binding publication fence", async () => {
    ...
    expect(() => registry.prepareSupervisionChildBinding(malformed, { agentKind: "pi", operatingPointId: "worker-pi", paneId: "bad\nid" })).toThrow(/SUPERVISION_BINDING_INVALID/u);

    const misaligned = await runningSupervisor(registry, { ...supervisorRequest, targets: [], target_generation_refs: [] });
    expect(() => registry.prepareSupervisionChildBinding(misaligned, { agentKind: "pi", operatingPointId: "worker-pi", paneId: "p1" })).toThrow(/SUPERVISION_REQUEST_INVALID/u);

    const jobId = await runningSupervisor(registry);
    const publication = registry.prepareSupervisionChildBinding(jobId, { agentKind: "pi", operatingPointId: "worker-pi", paneId: "p1" });
    ...
    const closedPublication = registry.prepareSupervisionChildBinding(closed, { agentKind: "pi", operatingPointId: "worker-pi", paneId: "p2" });
    registry.shutdown();
