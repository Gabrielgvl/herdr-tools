...
describe("Herdr compatibility preflight", () => {
  ...
  it("rejects the retired protocol before any prompt-capable preflight succeeds", async () => {
    const stale = JSON.stringify({ client: { version: "0.8.0", protocol: REQUIRED_HERDR_PROTOCOL - 1 }, server: { status: "running", version: "0.8.0", protocol: REQUIRED_HERDR_PROTOCOL - 1, compatible: true } });
    expect(() => parseHealth(stale)).toThrowError(expect.objectContaining({ code: "CLI_INCOMPATIBLE" }));
    await expect(preflightCompatibility({ runText: vi.fn(async () => stale) }, new AbortController().signal)).rejects.toMatchObject({ code: "CLI_INCOMPATIBLE" });
  });
  ...
  it("does not retain retired-protocol health evidence", () => {
    const stale = healthDetails({ version: "0.8.0", protocol: REQUIRED_HERDR_PROTOCOL - 1 }, { ...validHealthServer, protocol: REQUIRED_HERDR_PROTOCOL - 1 });
    const mapped = mapPreflightFailure(new CliProtocolError("CLI_INCOMPATIBLE", "preflight failed", { health: stale }));
    expect(mapped.details).not.toHaveProperty("health");
  });
