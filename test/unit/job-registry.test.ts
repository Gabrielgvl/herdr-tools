...
const provisionalRequest: SupervisorJobRequestSnapshot = {
  ...
  child: { agentName: "agy", agentKind: "agy", operatingPointId: "researcher-agy" },
...
describe("JobRegistry", () => {
  ...
  it("publishes AGY provisional supervision without exact coverage or cancellation", async () => {
    ...
    const publication = registry.prepareProvisionalSupervisionChildBinding(handle.jobId, { agentKind: "agy", operatingPointId: "researcher-agy" });
    ...
    expect(registry.get(handle.jobId)).toMatchObject({
      ...
      request: { targetIds: [], child: { agentName: "agy", agentKind: "agy", operatingPointId: "researcher-agy" } },
    ...
    expect(registry.get(handle.jobId)).toMatchObject({
      operation_phase: "running",
      request: { targetIds: [], child: { agentKind: "agy", operatingPointId: "researcher-agy" } },
    });
  ...
  it.each(["pi", "claude"] as const)("allows a %s-requested reservation to bind an AGY fallback provisionally", async (agentKind) => {
    ...
    const requestedOperatingPointId = `researcher-${agentKind}`;
    const handle = registry.register({
      ...provisionalRequest,
      child: { agentName: "worker", agentKind, operatingPointId: requestedOperatingPointId },
    }, async () => new Promise<never>(() => undefined));
    await vi.waitFor(() => expect(registry.get(handle.jobId)?.operation_phase).toBe("running"));

    const publication = registry.prepareProvisionalSupervisionChildBinding(handle.jobId, { agentKind: "agy", operatingPointId: "fallback-agy" });
    publication.commit();
    expect(registry.get(handle.jobId)?.request).toMatchObject({
      targetIds: [],
      child: { agentKind: "agy", operatingPointId: "fallback-agy", requestedAgentKind: agentKind, requestedOperatingPointId },
    });
    publication.rollback();
    expect(registry.get(handle.jobId)?.request).toMatchObject({ child: { agentKind, operatingPointId: requestedOperatingPointId } });
    registry.shutdown();
  });
  ...
  it.each(["pi", "claude"] as const)("allows an AGY-requested reservation to bind an exact %s fallback", async (agentKind) => {
    ...
    const operatingPointId = `researcher-${agentKind}`;
    const publication = registry.prepareSupervisionChildBinding(handle.jobId, { agentKind, operatingPointId, paneId: "p1" });
    publication.commit();
    expect(registry.get(handle.jobId)?.request).toMatchObject({
      targetIds: ["p1"],
      child: { agentKind, operatingPointId, requestedAgentKind: "agy", requestedOperatingPointId: "researcher-agy" },
    });
    publication.rollback();
    registry.shutdown();
  });
  ...
  it("publishes bounded semantic-review ownership independently and rejects late or wrong-kind writes", async () => {
    ...
    const supervisor = registry.register({
      ...
      child: { agentName: "one", agentKind: "pi", operatingPointId: "worker-pi" },
      settings: { reviewCadenceMinutes: 1, reviewerModel: "testmodel", reviewerThinking: "max" },
  ...
  it("drops a supervisor request's supervisionDigest at the public projection", () => {
    const digestRequest: SupervisorJobRequestSnapshot = {
      ...provisionalRequest,
      settings: {
  ...
  it("requires a live matching installed supervisor before publishing a binding", async () => {
    ...
    const closedPublication = registry.prepareProvisionalSupervisionChildBinding(closed.jobId, { agentKind: "agy", operatingPointId: "researcher-agy" });
    ...
    const missingPublication = registry.prepareProvisionalSupervisionChildBinding(missing.jobId, { agentKind: "agy", operatingPointId: "researcher-agy" });
    ...
    const invalidPublication = registry.prepareProvisionalSupervisionChildBinding(invalid.jobId, { agentKind: "agy", operatingPointId: "researcher-agy" });
    invalidPublication.commit();
    expect(() => invalidPublication.publish()).toThrow(/SUPERVISION_PUBLICATION_UNCONFIRMED/u);

    const exactRequest: SupervisorJobRequestSnapshot = { ...provisionalRequest, targets: ["worker"], child: { agentName: "worker", agentKind: "pi", operatingPointId: "worker-pi" } };
    ...
    const exactPublication = registry.prepareSupervisionChildBinding(degraded.jobId, { agentKind: "pi", operatingPointId: "worker-pi", paneId: "p1" });
  ...
  it("covers defensive provisional and exact binding transactions", async () => {
    ...
    const provisional = { agentKind: "agy" as const, operatingPointId: "researcher-agy" };
    ...
    expect(() => registry.prepareProvisionalSupervisionChildBinding(nonAgy.jobId, { agentKind: "pi", operatingPointId: "worker-pi" })).toThrow(/SUPERVISION_PROVISIONAL_INVALID/u);
    const malformed = await register();
    expect(() => registry.prepareProvisionalSupervisionChildBinding(malformed.jobId, { agentKind: "agy", operatingPointId: "" })).toThrow(/SUPERVISION_BINDING_INVALID/u);
    ...
    const first = registry.prepareProvisionalSupervisionChildBinding(transactional.jobId, { agentKind: "agy", operatingPointId: "fallback-agy" });
    const stale = registry.prepareProvisionalSupervisionChildBinding(transactional.jobId, provisional);
    stale.rollback();
    first.commit();
    expect(registry.get(transactional.jobId)?.request).toMatchObject({ child: { operatingPointId: "fallback-agy", requestedOperatingPointId: "researcher-agy" } });
    ...
    const installed: SupervisionJobView = {
      ...
      provisional: { agentName: "agy", agentKind: "agy", paneId: "p1", terminalId: "t1", operatingPointId: "researcher-agy", baseline: { state: "idle", stateChangeSeq: 1, revision: 1 } },
    };
    ...
    expect(() => registry.prepareSupervisionStrengthening(invalidStrengthening.jobId, { agentKind: "agy", operatingPointId: "researcher-agy", paneId: "p1" })).toThrow(/SUPERVISION_STRENGTHENING_INVALID/u);
    const unavailable = await register();
    registry.prepareProvisionalSupervisionChildBinding(unavailable.jobId, provisional).commit();
    expect(() => registry.prepareSupervisionStrengthening(unavailable.jobId, { agentKind: "agy", operatingPointId: "researcher-agy", paneId: "p1" })).toThrow(/SUPERVISION_PUBLICATION_UNCONFIRMED/u);

    const exactRequest: SupervisorJobRequestSnapshot = { ...provisionalRequest, child: { agentName: "worker", agentKind: "pi", operatingPointId: "worker-pi" } };
    const exact = await register(exactRequest);
    const exactFirst = registry.prepareSupervisionChildBinding(exact.jobId, { agentKind: "pi", operatingPointId: "worker-pi", paneId: "p1" });
    const exactStale = registry.prepareSupervisionChildBinding(exact.jobId, { agentKind: "pi", operatingPointId: "worker-pi", paneId: "p1" });
    exactFirst.commit();
