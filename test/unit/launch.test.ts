...
import { ProfileLaunchParamsSchema, renderAssignment, type LaunchAssignment, type LaunchParams } from "../../src/launch-schema.js";
...
const autoRequest = (overrides: Record<string, unknown> = {}): Record<string, unknown> => ({ name: "task", assignment: assign("go"), ...overrides });
...
describe("herdr_launch auto batch", () => {
  it("explicit mode performs zero Router and zero log calls without an API key", async () => {
    ...
    const result = await tool.execute("id", { name: "worker", profile: "worker", assignment: assign("go") }, new AbortController().signal, undefined, extensionContext);
  ...
  it("produces a per-child AGY_UNQUALIFIED failure for an AGY selection", async () => {
    const harness = makeBatchCli();
    ...
    expect(result.details?.outcome).toBe("partial");
    const children = result.details?.children ?? [];
    expect(children[0]).toMatchObject({ name: "task-scout-1", profile: "scout-agy", status: "failed", failure: { code: "AGY_UNQUALIFIED" } });
