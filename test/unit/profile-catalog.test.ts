...
describe("profile catalog", () => {
  it("parses strict YAML, typed runtime defaults, and profile metadata", () => {
    ...
    const parsed = parseProfile(profileText("worker").replace("thinking: low", "thinking: low\n  tools: [read, grep]"), source(root, "worker"));
    expect(parsed).toMatchObject({ name: "worker", timeoutMinutes: 30, sessionPersistence: false, fallbackProfiles: [], runtime: { kind: "pi", thinking: "low", tools: ["read", "grep"] } });
  ...
  it("loads bundled profiles from the package scope", async () => {
    ...
    expect(catalog.effective.get("worker-pi")?.source.scopeRoot).toContain("herdr-tools-profiles");
  });
});
