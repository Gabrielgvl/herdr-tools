...
describe("supervisor-aware job surfaces", () => {
  ...
  it("shows supervisor rows and unobserved counts in the active job list", () => {
    ...
    const registered = registry.register(
      { kind: "supervisor", label: "supervise worker", targets: ["worker"], targetIds: [], child: { agentName: "worker", agentKind: "pi", operatingPointId: "worker-pi" }, settings: { reviewCadenceMinutes: 5, reviewerModel: "typesafe/jev-latest", reviewerThinking: "max" } },
  ...
  it("settles a supervisor whose runner throws", async () => {
    ...
    const request = { kind: "supervisor" as const, label: "supervise worker", targets: ["worker"], targetIds: [], child: { agentName: "worker", agentKind: "pi", operatingPointId: "worker-pi" }, settings: { reviewCadenceMinutes: 5, reviewerModel: "typesafe/jev-latest", reviewerThinking: "max" as const } };
...
describe("supervision runtime seams", () => {
  ...
  it("builds the shared Jev reviewer even when a model service is supplied", async () => {
    const { SupervisionRegistry } = await import("../../src/supervision/registry.js");
    const { TypeSafeSupervisionReviewer } = await import("../../src/supervision/reviewer.js");
    const supervision = new SupervisionRegistry({
    ...
    await expect((supervision as unknown as { reviewer(): Promise<unknown> }).reviewer()).resolves.toBeInstanceOf(TypeSafeSupervisionReviewer);
    await supervision.shutdown();
  });
});
