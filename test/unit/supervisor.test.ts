...
describe("supervisor binding", () => {
  ...
  it("binds to the proven occupant and anchors on its revision", async () => {
    ...
    expect(view.child).toEqual({ agentName: "worker", agentKind: "pi", paneId: "p1", terminalId: "t1", operatingPointId: "worker-pi" });
  ...
  it("publishes AGY provisional evidence, keeps the observer, then strengthens atomically", async () => {
    ...
    const h = harness({ child: { agentName: "worker", agentKind: "agy", operatingPointId: "researcher-agy" }, snapshots: [baseline, exact] });
    const provisional: ProvisionalSupervisionBinding = { identity: agyIdentity, operatingPointId: "researcher-agy", baseline: { state: "idle", stateChangeSeq: 4, revision: 2 } };
    const events: string[] = [];
    ...
    await h.supervisor.strengthen({ identity: exactIdentity, operatingPointId: "researcher-agy" }, { commit: () => { events.push(`exact-commit:${h.supervisor.view().state}`); }, rollback: vi.fn(), publish: () => { events.push(`exact-publish:${h.supervisor.view().state}`); } });
    ...
    expect(h.supervisor.view()).toMatchObject({ state: "active", child: { paneId: "p1", terminalId: "t1", agentKind: "agy", operatingPointId: "researcher-agy" }, status: "working" });
  ...
  it("retains ordered lifecycle events before the fresh AGY strengthening read", async () => {
    ...
    const h = harness({ child: { agentName: "worker", agentKind: "agy", operatingPointId: "researcher-agy" }, snapshots: [snapshot([agyPaneRecord()], [agyAgentRecord()]), exact] });
  ...
  it.each([
    ...
    await expect(h.supervisor.strengthen({ identity: exactAgyIdentity, operatingPointId: "researcher-agy" }, publication))
  ...
  it("drains events admitted during the fresh AGY strengthening read before commit", async () => {
    ...
    const h = harness({ child: { agentName: "worker", agentKind: "agy", operatingPointId: "researcher-agy" }, snapshots: [baseline, fresh] });
    await h.supervisor.bindProvisional({ identity: agyIdentity, operatingPointId: "researcher-agy", baseline: { state: "idle", stateChangeSeq: 4, revision: 2 } });
    const commit = vi.fn();
    let published: ReturnType<Supervisor["view"]> | undefined;
    const strengthening = h.supervisor.strengthen({ identity: { ...agyIdentity, agentSession: agySession }, operatingPointId: "researcher-agy" }, { commit, rollback: vi.fn(), publish: () => { published = h.supervisor.view(); } });
  ...
  it("rejects a move admitted after the strengthening drain empties but before exact commit", async () => {
    ...
    const publication = { commit: vi.fn(), rollback: vi.fn(), publish: vi.fn() };
    const failure = await h.supervisor.strengthen({ identity: { ...agyIdentity, agentSession: agySession }, operatingPointId: "researcher-agy" }, publication)
  ...
  it("pins the first native session seen before strengthening", async () => {
    ...
    await h.supervisor.bindProvisional({ identity: agyIdentity, operatingPointId: "researcher-agy", baseline: { state: "idle", stateChangeSeq: 4, revision: 2 } });
    await h.supervisor.onEvent(paneEvent("pane_updated", agyPaneRecord({ agent_session: agySession, agent_status: "working", revision: 3, state_change_seq: 5 })));
    const failure = await h.supervisor.strengthen({ identity: { ...agyIdentity, agentSession: otherSession }, operatingPointId: "researcher-agy" })
      .catch((error: SupervisionBindError) => error);
    expect(failure).toBeInstanceOf(SupervisionBindError);
  ...
  it("binds without a state_change_seq rather than defaulting one", async () => {
    const h = harness({ snapshots: [snapshot([paneRecord()])] });
    await expect(h.supervisor.bind({ identity, operatingPointId: "worker-pi" })).resolves.toBeUndefined();
    expect(h.timerArmed()).toBe(false);

    const regressed = harness({ snapshots: [snapshot([paneRecord({ stateChangeSeq: 3 })])] });
    await expect(regressed.supervisor.bind({ identity, operatingPointId: "worker-pi", stateChangeSeq: 4 }))
      .rejects.toMatchObject({ details: { cause: "lifecycle_regressed" } });

    const stopped = harness();
    stopped.supervisor.shutdown();
    await expect(stopped.supervisor.bind({ identity, operatingPointId: "worker-pi" })).rejects.toMatchObject({ details: { cause: "bind_already_attempted" } });
  });
  ...
  it("refuses to bind when authoritative state is unreadable, absent, or a different agent", async () => {
    ...
    await expect(unreadable.supervisor.bind({ identity, operatingPointId: "worker-pi" })).rejects.toBeInstanceOf(SupervisionBindError);
    expect(unreadable.observers).toBe(0);

    const missing = harness({ snapshots: [snapshot([], [])] });
    await expect(missing.supervisor.bind({ identity, operatingPointId: "worker-pi" })).rejects.toThrow(/no valid unique authoritative occupant/u);

    const invalid = harness({ snapshots: [snapshot([paneRecord(), paneRecord()])] });
    await expect(invalid.supervisor.bind({ identity, operatingPointId: "worker-pi" })).rejects.toThrow(/no valid unique authoritative occupant/u);

    const agentFree = harness({ snapshots: [snapshot([paneRecord()], [])] });
    await expect(agentFree.supervisor.bind({ identity, operatingPointId: "worker-pi" })).rejects.toThrow(/no valid unique authoritative occupant/u);

    const replaced = harness({ snapshots: [snapshot([paneRecord({ agentSession: { ...session, value: "other" } })])] });
    const failure = await replaced.supervisor.bind({ identity, operatingPointId: "worker-pi" }).catch((error: SupervisionBindError) => error);
    expect(failure).toBeInstanceOf(SupervisionBindError);
  ...
  it("queues events that land before the anchor exists and commits only after draining them", async () => {
    ...
    const binding = h.supervisor.bind({ identity, operatingPointId: "worker-pi" }, publication);
  ...
  it("rejects when queued closure or replacement evidence settles during bind", async () => {
    ...
    for (const scenario of scenarios) {
      ...
      const binding = scenario.h.supervisor.bind({ identity, operatingPointId: "worker-pi" }, publication);
  ...
  it("holds the commit for closure evidence admitted during the drain", async () => {
    ...
    const commit = vi.fn();
    const rollback = vi.fn();
    const binding = h.supervisor.bind({ identity, operatingPointId: "worker-pi" }, { commit, rollback, publish: vi.fn() });
  ...
  it("does not reset a shutdown settlement when binding read fails", async () => {
    ...
    await expect(h.supervisor.bind({ identity, operatingPointId: "worker-pi" })).rejects.toBeInstanceOf(SupervisionBindError);
  ...
  it("rolls back a failed request publication and keeps the reservation unbound", async () => {
    ...
    const failure = await h.supervisor.bind({ identity, operatingPointId: "worker-pi" }, publication).catch((error: SupervisionBindError) => error);
    expect(failure).toBeInstanceOf(SupervisionBindError);
    ...
    expect(h.supervisor.childLive()).toBe(false);
    expect(h.observers).toBe(0);
    await expect(h.supervisor.bind({ identity, operatingPointId: "worker-pi" })).rejects.toThrow(/single-use/u);
    h.supervisor.release("bind_publication_failed");
  });
});

describe("AGY provisional supervision failures", () => {
  const agyChild = { agentName: "worker", agentKind: "agy", operatingPointId: "researcher-agy" };
  ...
  it("validates every provisional binding field and remains single-use", async () => {
    ...
    const malformed: ProvisionalSupervisionBinding[] = [
      ...
      ...([1, "", "bad\n"] as unknown[]).map((operatingPointId) => ({ ...provisionalBinding, operatingPointId } as ProvisionalSupervisionBinding)),
  ...
  it.each(["pi", "claude"] as const)("allows a %s reservation to bind and strengthen an AGY fallback", async (agentKind) => {
    const h = harness({
      child: { agentName: "worker", agentKind, operatingPointId: `researcher-${agentKind}` },
      snapshots: [snapshot([agyPaneRecord()], [agyAgentRecord()]), agyExactSnapshot()],
    });
    await h.supervisor.bindProvisional({ ...provisionalBinding, operatingPointId: "fallback-agy" });
    expect(h.supervisor.view()).toMatchObject({ state: "provisional", provisional: { agentKind: "agy", operatingPointId: "fallback-agy", requestedOperatingPointId: `researcher-${agentKind}` } });
    await h.supervisor.strengthen({ identity: exactAgyIdentity, operatingPointId: "fallback-agy" });
    expect(h.supervisor.view()).toMatchObject({ state: "active", child: { agentKind: "agy", operatingPointId: "fallback-agy", requestedAgentKind: agentKind, requestedOperatingPointId: `researcher-${agentKind}` } });
    h.supervisor.shutdown();
  });

  it("projects the requested profile beside a provisional fallback", async () => {
    const h = agyHarness([snapshot([agyPaneRecord()], [agyAgentRecord()])]);
    await bindAgy(h, { ...provisionalBinding, operatingPointId: "fallback-agy" });
    expect(h.supervisor.view()).toMatchObject({ provisional: { operatingPointId: "fallback-agy", requestedOperatingPointId: "researcher-agy" } });
    h.supervisor.shutdown();
  });
});
...
describe("AGY supervision strengthening failures", () => {
  ...
  it("rejects repeated and mismatched strengthening requests", async () => {
    await expect(agyHarness().supervisor.strengthen({ identity: exactAgyIdentity, operatingPointId: "researcher-agy" }))
      .rejects.toMatchObject({ details: { cause: "strengthen_already_attempted" } });

    for (const binding of [
      { identity: { ...exactAgyIdentity, agentKind: "pi" }, operatingPointId: "researcher-agy" },
      { identity: { ...exactAgyIdentity, paneId: "p2" }, operatingPointId: "researcher-agy" },
    ...
    stopped.supervisor.shutdown();
    await expect(stopped.supervisor.strengthen({ identity: exactAgyIdentity, operatingPointId: "researcher-agy" }))
      .rejects.toMatchObject({ details: { cause: "strengthen_already_attempted" } });

    const strengthened = await provisional([agyExactSnapshot()]);
    await strengthened.supervisor.strengthen({ identity: exactAgyIdentity, operatingPointId: "researcher-agy" });
    await expect(strengthened.supervisor.strengthen({ identity: exactAgyIdentity, operatingPointId: "researcher-agy" }))
      .rejects.toMatchObject({ details: { cause: "strengthen_already_attempted" } });
  ...
  it("rejects every fresh strengthening evidence failure and rolls back", async () => {
    ...
    for (const [evidence, requestedIdentity, cause] of cases) {
      ...
      const rollback = vi.fn();
      await expect(h.supervisor.strengthen({ identity: requestedIdentity, operatingPointId: "researcher-agy" }, { commit: vi.fn(), rollback, publish: vi.fn() }))
        .rejects.toMatchObject({ details: { cause } });
    ...
    await bindAgy(pinned);
    await expect(pinned.supervisor.strengthen({ identity: { ...agyIdentity, agentSession: otherSession }, operatingPointId: "researcher-agy" }))
  ...
  it("carries prior provisional failure into strengthening and refuses release", async () => {
    ...
    await expect(h.supervisor.strengthen({ identity: exactAgyIdentity, operatingPointId: "researcher-agy" }, { commit: vi.fn(), rollback, publish: vi.fn() }))
  ...
  it("rejects malformed or regressed evidence admitted during strengthening", async () => {
    ...
    for (const [overrides, cause] of cases) {
      ...
      const strengthening = h.supervisor.strengthen({ identity: exactAgyIdentity, operatingPointId: "researcher-agy" });
      await Promise.resolve();
  ...
  it("replays strengthened transitions without false same-revision gaps", async () => {
    ...
    const fresh = new Promise<HerdrSnapshot>((resolve) => { resolveFresh = resolve; });
    const h = await provisional([fresh]);
    const strengthening = h.supervisor.strengthen({ identity: exactAgyIdentity, operatingPointId: "fallback-agy" });
    await Promise.resolve();
    ...
    expect(h.supervisor.view()).toMatchObject({
      state: "active",
      status: "working",
      child: { operatingPointId: "fallback-agy", requestedOperatingPointId: "researcher-agy" },
  ...
  it("fails closed when a strengthening candidate disappears or settles", async () => {
    for (const [mutation, cause] of [
      ...
      await expect(h.supervisor.strengthen({ identity: exactAgyIdentity, operatingPointId: "researcher-agy" }))
        .rejects.toMatchObject({ details: { cause } });
      h.supervisor.shutdown();
    }
  });
  ...
  it("covers the serialized strengthening commit guards", async () => {
    ...
    await expect(queued.supervisor.strengthen({ identity: exactAgyIdentity, operatingPointId: "researcher-agy" }))
      .rejects.toMatchObject({ details: { cause: "publication_failed" } });
    ...
    await expect(drain.supervisor.strengthen({ identity: exactAgyIdentity, operatingPointId: "researcher-agy" }))
...
describe("supervisor folding", () => {
  ...
  async function exactAgyBound(after: HerdrSnapshot[] = []): Promise<Harness> {
    const child = { agentName: "worker", agentKind: "agy", operatingPointId: "researcher-agy" };
  ...
  it("accepts a Pi same-revision transition when its authoritative lifecycle sequence advances", async () => {
    ...
    await unanchored.supervisor.bind({ identity, operatingPointId: "worker-pi" });
  ...
  it("keeps same-revision Pi and Claude transitions gap-visible without lifecycle evidence", async () => {
    ...
    for (const [agentKind, agentSession] of [["pi", session], ["claude", claudeSession]] as const) {
      ...
      await h.supervisor.bind({ identity: boundIdentity, operatingPointId: `worker-${agentKind}` });
...
describe("supervisor review cadence", () => {
  ...
  it("does not review a child that stopped working or a settled supervisor", async () => {
    ...
    await stopped.supervisor.bind({ identity, operatingPointId: "worker-pi" });
...
describe("the ADR-036 evidence cadence", () => {
  ...
  it("keeps a typed trace failure silent as reviewer_unavailable and retries next cadence", async () => {
    ...
    await h.supervisor.bind({ identity: { ...identity, agentSession: badSession }, operatingPointId: "worker-pi" });
    h.fireTimer();
...
describe("the supervisor job port", () => {
  ...
  it("settles on shutdown and on a released reservation, and refuses to re-settle", async () => {
    ...
    await shutdown.supervisor.bind({ identity, operatingPointId: "worker-pi" });
    ...
    await settledThenStopped.supervisor.bind({ identity, operatingPointId: "worker-pi" });
    ...
    await releasedAfterSettle.supervisor.bind({ identity, operatingPointId: "worker-pi" });
  ...
  it("projects fallback child identity fields and settled omissions", async () => {
    const fallback = harness({
      child: { agentName: "worker", agentKind: "claude", operatingPointId: "requested-claude" },
      snapshots: [snapshot([paneRecord()])],
    });
    await fallback.supervisor.bind({ identity, operatingPointId: "fallback-pi" });
    expect(fallback.supervisor.view()).toMatchObject({
      child: { operatingPointId: "fallback-pi", requestedOperatingPointId: "requested-claude", requestedAgentKind: "claude" },
    });
...
describe("self-close wake suppression", () => {
  ...
  it("keeps identity_replaced and identity_lost wakes without spending the marker", async () => {
    ...
    await replaced.supervisor.bind({ identity, operatingPointId: "worker-pi" });
    tracker.begin("p1")(true);
    await replaced.supervisor.onEvent(paneEvent("pane_updated", paneRecord({ terminalId: "t9", revision: 6 })));
    expect(types(replaced.wakes)).toEqual(["identity_replaced"]);

    const lost = harness({ selfClose: tracker, snapshots: [snapshot([paneRecord()])] });
    await lost.supervisor.bind({ identity, operatingPointId: "worker-pi" });
  ...
  it("falls back to waking when the tracker throws or its decision rejects", async () => {
    ...
    await deferred.supervisor.bind({ identity, operatingPointId: "worker-pi" });
