...
function thinEvent(kind: string, paneId = "p1"): SupervisionSocketEvent {
  return { kind: "event", event: kind as SupervisionSocketEvent["event"], data: { type: kind, pane_id: paneId, workspace_id: "w1" } };
}
...
describe("supervisor binding", () => {
  it("binds to the proven occupant and anchors on its revision", async () => {
    ...
    await h.supervisor.bind({ identity, stateChangeSeq: 4 });
  ...
  it("binds without a state_change_seq rather than defaulting one", async () => {
    ...
    await expect(h.supervisor.bind({ identity })).resolves.toBeUndefined();
  ...
  it("refuses to bind when authoritative state is unreadable, absent, or a different agent", async () => {
    ...
    await expect(unreadable.supervisor.bind({ identity })).rejects.toBeInstanceOf(SupervisionBindError);
    expect(unreadable.observers).toBe(0);

    const missing = harness({ snapshots: [snapshot([])] });
    await expect(missing.supervisor.bind({ identity })).rejects.toThrow(/no unique authoritative occupant/u);

    const replaced = harness({ snapshots: [snapshot([paneRecord({ agentSession: { ...session, value: "other" } })])] });
    await expect(replaced.supervisor.bind({ identity })).rejects.toThrow(/could not prove the launched identity/u);
    const failure = await replaced.supervisor.bind({ identity }).catch((error: SupervisionBindError) => error);
  ...
  it("queues events that land before the anchor exists and folds them after", async () => {
    ...
    const binding = h.supervisor.bind({ identity });
...
describe("supervisor folding", () => {
  ...
  it("reconciles rather than concluding from a thin or unprovable event", async () => {
    ...
    const malformed = await bound({ snapshots: [snapshot([paneRecord({ status: "working", revision: 6 })])] });
    await malformed.supervisor.onEvent(paneEvent("pane_updated", { pane_id: "p1" }));
    expect(malformed.wakes).toHaveLength(0);
    expect(malformed.supervisor.view().status).toBe("working");
  });
...
describe("supervisor pane moves", () => {
  ...
  it("settles identity_lost when a move is not atomic or cannot be proven", async () => {
    const notAtomic = await bound();
    await notAtomic.supervisor.onEvent(paneEvent("pane_moved", paneRecord({ paneId: "p2", revision: 6 })));
    expect(types(notAtomic.wakes)).toEqual(["identity_lost"]);
    expect(await notAtomic.supervisor.run()).toEqual({ outcome: "identity_lost", reason: "move_not_atomic" });

    const wrongPrevious = await bound();
    await wrongPrevious.supervisor.onEvent(paneEvent("pane_moved", paneRecord({ paneId: "p2", revision: 6 }), { previous_pane_id: "p9" }));
...
describe("supervisor review cadence", () => {
  ...
  it("does not review a child that stopped working or a settled supervisor", async () => {
    ...
    await stopped.supervisor.bind({ identity });
    stopped.fireTimer();
...
describe("the supervisor job port", () => {
  ...
  it("settles on shutdown and on a released reservation, and refuses to re-settle", async () => {
    ...
    await shutdown.supervisor.bind({ identity });
    ...
    await settledThenStopped.supervisor.bind({ identity });
    ...
    await releasedAfterSettle.supervisor.bind({ identity });
